 ...
 * متضمن: لغات، 99 اسماً، مشغل سلس، تقويم سيستاني مضبوط تماماً، ونظام قراءة إشعارات من فايربيس.
 */
...
/* ==========================================
   نظام قراءة الإشعارات السحابية (من فايربيس)
   ========================================== */
function renderNotifications() {
  const container = document.getElementById("notifications-list");
  if (!container) return;
  
  // إظهار علامة تحميل ريثما تصل البيانات من الإنترنت
  container.innerHTML = `<div style="text-align: center; padding: 20px; color: var(--primary);"><i class="fa-solid fa-spinner fa-spin"></i> جاري جلب الإشعارات...</div>`;
  
  // استدعاء دالة فايربيس لجلب الرسائل
  if (window.fetchFirebaseNotifications) {
    window.fetchFirebaseNotifications();
  } else {
    container.innerHTML = `<div style="text-align: center; color: red;">تعذر الاتصال بالخادم.</div>`;
  }
}

// هذه الدالة تستلم الرسائل من فايربيس وتطبعها في الشاشة
window.displayFetchedNotifications = function(messages) {
  const container = document.getElementById("notifications-list");
  if (!container) return;
  ...
  // عرض الرسائل (نعكسها لتظهر الأحدث أولاً)
