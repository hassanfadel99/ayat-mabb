/**
 * تطبيق آيات - الملف البرمجي الكامل والمحمي (نسخة الأداء السريع)
 */
...
window.renderStories = function(filterType) {
  ...
  storiesToRender.forEach((vid, index) => {
    ...
    // 🌟 التحديث الأهم: استخدام preload="metadata" لتسريع الإنترنت ومنع التشنج
    container.innerHTML += `
      <div class="story-card">
        <div class="story-video-container">
          <video id="story-vid-${index}" class="story-video" src="${vid.url}" loop playsinline webkit-playsinline preload="metadata"></video>
...
// 🌟 حماية زر المفضلة من التشنج ومنع التداخل
window.toggleFavorite = function(event, url, index) {
    if(event) event.stopPropagation(); // منع تداخل اللمس مع الفيديو
    triggerHaptic();
...
// 🌟 تخفيض حساسية التشغيل لـ 50% لتعمل بسلاسة تامة دون تشنج
