/** 
 * تطبيق آيات - الملف البرمجي الشامل (النسخة 2.9)
 * تحسينات البث المباشر (CORS Bypass) وإجبار استمرار الصوت في الخلفية
 */ 

let APP_CURRENT_VERSION = 2.9; 
...
window.startApp = function () { 
  ...
  if (splash) { 
    ...
    setTimeout(() => { 
      ...
      // تهيئة صامتة للصوت لدعم الخلفية في المتصفحات
      try { 
...
// ==========================================
function toggleColorSlider() { triggerHaptic(); const hc = document.getElementById("hue-slider-container"); if (hc) hc.classList.toggle("hidden"); } 
function setAppMode(mode) { 
  triggerHaptic(); const nBtn = document.getElementById("mode-night-btn"); const dBtn = document.getElementById("mode-day-btn"); const hc = document.getElementById("hue-slider-container"); 
  if (mode === "night") { document.body.classList.add("dark-mode"); localStorage.setItem("ayat_darkmode", "true"); if (nBtn) nBtn.classList.add("active"); if (dBtn) dBtn.classList.remove("active"); if (hc) hc.classList.add("hidden"); } 
  else { document.body.classList.remove("dark-mode"); localStorage.setItem("ayat_darkmode", "false"); if (dBtn) dBtn.classList.add("active"); if (nBtn) nBtn.classList.remove("active"); applyHueColor(localStorage.getItem("ayat_hue") || 345, false); } 
} 
...
// ==========================================
// 🌟 1. نظام إشعارات شاشة القفل والخلفية (Media Session)
// ==========================================
...
// ==========================================
// 🌟 2. نظام البث المرئي المباشر المقاوم للحماية (HLS)
// ==========================================
...
function playLiveChannel(url, btnElement) {
    ...
    // طريقة 1: محاولة تشغيل البث كملف فيديو طبيعي (يتخطى CORS في هواتف الآيفون وبعض الأندرويد)
    if (video.canPlayType('application/vnd.apple.mpegurl')) {
        video.src = url;
        video.addEventListener('loadedmetadata', function() {
            loader.classList.add('hidden'); 
            video.play().catch(e => showToast("تعذر التشغيل: المتصفح يمنع البث مؤقتاً"));
        });
    } 
    // طريقة 2: استخدام مكتبة HLS.js للأجهزة الأخرى
        ...
        hlsInstance.on(Hls.Events.MANIFEST_PARSED, function () {
            loader.classList.add('hidden'); 
            video.play().catch(e => showToast("الحماية تمنع البث. سيعمل في تطبيق الأندرويد."));
        });
        hlsInstance.on(Hls.Events.ERROR, function (event, data) {
            if (data.fatal) {
                loader.innerHTML = "المتصفح يمنع البث (CORS). سيعمل في التطبيق لاحقاً.";
            }
        });
    } else {
...
// ==========================================
// 🌟 4. مشغل القرآن الكريم (الشريط العائم)
// ==========================================
...
// ==========================================
// 🌟 5. مكتبة الأدعية والمشغل الكامل (Lark Style)
// ==========================================
...
function playCustomAudioFile(fileName, title, reader, cover) {
    ...
    libAudioEl.play().then(() => { updateExternalMediaSession(title, reader, cover); }).catch(e => showToast("الرجاء رفع الملفات لجيت هب وتجربتها لاحقاً"));
...
// ==========================================
// 🌟 نظام مواقيت الصلاة والأذان (المفلتر والمحسن)
// ==========================================
...
function triggerAdhanAlert(prayerName) { 
  ...
  try { stopQuranAudio(); } catch (e) {} try { stopLibAudio(); } catch (e) {} 
...
function stopAdhan() { 
  triggerHaptic(); const popup = document.getElementById("adhan-alert-popup"); if (popup) popup.classList.add("hidden-popup"); 
  const adhanAudio = document.getElementById("adhan-audio"); if (adhanAudio) { adhanAudio.pause(); adhanAudio.currentTime = 0; } 
} 

// ==========================================
// 🌟 باقي الميزات (الستوريات، المسبحة، الحاسبة، الختمة العائلية)
// ==========================================
...
function executeDelete() { triggerHaptic(); if (deleteActionTarget === "single" && deleteTargetIndex !== null) { const h = JSON.parse(localStorage.getItem("financial_history_records")) || []; h.splice(deleteTargetIndex, 1); localStorage.setItem("financial_history_records", JSON.stringify(h)); renderFinancialHistory(); showToast("تم حذف السجل"); } else if (deleteActionTarget === "all") { localStorage.removeItem("financial_history_records"); renderFinancialHistory(); showToast("تم المسح بالكامل"); } else if (deleteActionTarget === "leave_fk_room") { if (currentFkRoomId && database) { database.ref('family_khatmas/' + currentFkRoomId).once('value').then(snapshot => { const room = snapshot.val(); if (room && room.creator === fkMyUid) { database.ref('family_khatmas/' + currentFkRoomId).remove(); } else if (room) { database.ref('family_khatmas/' + currentFkRoomId + '/participants/' + fkMyUid).remove(); } currentFkRoomId = null; localStorage.removeItem('fk_current_room'); showToast("تم الخروج من الختمة بنجاح"); switchFkTab('create'); }); } } else if (deleteActionTarget === "single_fk_history" && deleteTargetIndex !== null) { let history = JSON.parse(localStorage.getItem('fk_history')) || []; history.splice(deleteTargetIndex, 1); localStorage.setItem('fk_history', JSON.stringify(history)); loadFkHistory(); showToast("تم مسح السجل بنجاح"); } closeDeletePopup(); } 
