 ...
 * متضمن: القوائم كاملة، الستوريات (تيك توك Auto-play)، شريط التحميل، والمشاركة الأصلية.
 */
...
const i18n = {
  ...
  en: {
    ...
    about_desc: "Ayat is your digital companion for an integrated spiritual journey.",
  ...
  ku: {
    ...
    copy: "کۆپی کردن", close: "داخستن", share: "بڵاوکردنەوەی ئەپ", stories: "ستۆری قورئان", stories_desc: "خوێندنەوەی کورت"
  },
  fa: {
    ...
    copy: "کپی متن", close: "بستن", share: "اشتراک برنامه", stories: "استوری های قرآن", stories_desc: "تلاوت های کوتاه"
  }
};

// تم استرجاع الـ 99 اسم بالكامل كما طلبت
const asmaUlHusna = [
...
// تم استرجاع الأذكار بالكامل كما طلبت
...
/* ==========================================
   وظيفة مشاركة التطبيق (واجهة التطبيقات الأصلية)
   ========================================== */
function shareApp() {
  triggerHaptic();
  const shareData = {
  ...
  if (navigator.share) {
    // هذا الكود سيستدعي نافذة النظام الأساسية (واتساب، تيليجرام وغيرها) 
    navigator.share(shareData).catch((error) => console.log('خطأ في المشاركة:', error));
  } else {
    // هذه الرسالة تظهر فقط إذا كنت تستخدم موقع CodePen أو متصفح قديم لا يدعم المشاركة
    alert("ملاحظة: نافذة المشاركة ستعمل عند تثبيت التطبيق (APK) أو فتحه من هاتف حقيقي. تم نسخ الرابط كبديل.");
...
/* ==========================================
   نظام الستوريات مع ميزة التشغيل التلقائي (Auto-play)
   ========================================== */
function setupStoriesListener() {
  ...
  database.ref('stories').on('value', (snapshot) => {
    ...
    // تفعيل ميزة التشغيل التلقائي عند التمرير
    setTimeout(setupAutoPlayObserver, 500);
  });
}
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        ...
        if (entry.isIntersecting) {
          // عندما يصبح الفيديو في منتصف الشاشة، يشتغل تلقائياً
          vid.play().then(() => {
             if(overlay) overlay.classList.add('playing');
          }).catch(e => console.log("المتصفح يطلب تفاعل المستخدم أولاً"));
        } else {
          // عندما يخرج من الشاشة يتوقف ويعود للبداية
          vid.pause();
          vid.currentTime = 0;
          if(overlay) overlay.classList.remove('playing');
        }
      });
    }, { threshold: 0.6 }); // يعمل عندما يكون 60% من الفيديو ظاهراً بالشاشة
    videos.forEach(vid => observer.observe(vid));
  }
}

/* ==========================================
   ميزة التحميل المباشر مع ظهور شريط نسبة التحميل
   ========================================== */
async function directDownloadVideo(url) {
  ...
  textEl.innerText = "بدأ التحميل... 0%";
  barEl.style.width = "0%";
  ...
  try {
    ...
    // قراءة أجزاء الفيديو تدريجياً وتحديث الشريط
    while (true) {
      ...
      if (total) {
         ...
         textEl.innerText = `جاري التحميل... ${percent}%`;
         barEl.style.width = `${percent}%`;
      } else {
         textEl.innerText = `جاري التحميل... ${(loaded/1024/1024).toFixed(1)} MB`;
         barEl.style.width = `50%`;
      }
    }

    // تجميع الأجزاء وإنشاء ملف في الهاتف
    ...
    // إغلاق النافذة
    textEl.innerText = "تم الحفظ في الهاتف بنجاح! ✅";
    barEl.style.width = "100%";
    setTimeout(() => overlay.classList.add('hidden-popup'), 2000);

  } catch (error) {
    console.error(error);
    overlay.classList.add('hidden-popup');
    window.open(url, "_blank"); // خطة بديلة إذا كان هناك مانع أمني
  }
}
