 ...
 * النسخة المحدثة (1.6) - أزرار مخصصة للإشعارات + الختمة العائلية + الأذكار الصوتية المنفصلة
 */ 
...
// ==========================================
// 🌟 الأذكار الصوتية المنفصلة (بدون إنترنت) 🌟
// ==========================================
// تعريف المتغير مرة واحدة فقط لضمان عدم ظهور خطأ has already been declared
...
window.toggleAudioAzkar = function(type) {
    ...
    // إرجاع الأيقونات للحالة الطبيعية
    ...
    // تشغيل الملف المطلوب
    audioPlayerObject.src = type + ".mp3"; // إما sabah.mp3 أو massa.mp3
    audioPlayerObject.play().then(() => {
        currentAudioType = type;
        showToast(type === 'sabah' ? "جاري تشغيل أذكار الصباح..." : "جاري تشغيل أذكار المساء...");
        
        // تغيير أيقونة الزر الذي يعمل حالياً لزر إيقاف
        ...
        showToast("تنبيه: تأكد من رفع ملف " + type + ".mp3 في الموقع");
    });
};

// عند انتهاء المقطع، إرجاع الأيقونات لشكلها الطبيعي
...
function startAudio() { 
  triggerHaptic(); const readerSelect = document.getElementById("reader-select"); if (!quranAudioEl || !readerSelect) return; 
  quranAudioEl.src = `https://cdn.islamic.network/quran/audio-surah/128/${readerSelect.value}/${currentSurah}.mp3`; quranAudioEl.play(); 
} 
...
if (quranAudioEl) { 
  ...
  if (seekbar) { 
    const startDrag = () => { seekbar.isDragging = true; }; const endDrag = () => { seekbar.isDragging = false; quranAudioEl.currentTime = seekbar.value; }; 
    seekbar.addEventListener("mousedown", startDrag); seekbar.addEventListener("touchstart", startDrag, { passive: true }); seekbar.addEventListener("mouseup", endDrag); seekbar.addEventListener("touchend", endDrag); 
  ...
  quranAudioEl.onplay = () => { 
    ...
    const icon = document.getElementById("f-audio-icon"); if (icon) icon.classList.add("fa-spin"); animationFrameId = requestAnimationFrame(updateSeekbarSmoothly); 
  }; 
  quranAudioEl.onpause = () => { 
    ...
    const icon = document.getElementById("f-audio-icon"); if (icon) icon.classList.remove("fa-spin"); cancelAnimationFrame(animationFrameId); 
  }; 
...
function stopAudio() { 
  triggerHaptic(); if (quranAudioEl) { quranAudioEl.pause(); quranAudioEl.currentTime = 0; cancelAnimationFrame(animationFrameId); } 
  const player = document.getElementById("floating-audio-player"); if (player) player.classList.add("hidden"); 
} 
...
function navigatePage(dir) { 
  triggerHaptic(); if (dir === 1 && currentPage < pages.length - 1) { currentPage++; renderPage(); } else if (dir === -1 && currentPage > 0) { currentPage--; renderPage(); } 
} 
...
function resumeLastReading() { 
  const d = localStorage.getItem("lastQuranProgress"); if (d) { const p = JSON.parse(d); showSection("quran-section"); selectSurah(p.surahNum, p.surahName, p.pageIndex); } 
} 
...
function createFamilyRoom() { 
  ...
  const roomData = { 
    code: roomId, creator: fkMyUid, khatmaName: finalKhatmaName, type: type, status: "waiting", participants: {}, 
  }; 
...
function renderPortionsGrid(room) { 
  ...
  gridMine.innerHTML = ""; listAll.innerHTML = ""; 
  const totalItems = room.type === "ajza" ? 30 : 114; 
