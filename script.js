/** 
 * تطبيق آيات - الملف البرمجي الشامل (النسخة 2.8)
 * متوافق تماماً مع تصميم Lark Player الجديد والتبويبات الجانبية
 */ 

let APP_CURRENT_VERSION = 2.8; 
...
// ==========================================
// 🌟 إشعارات شاشة القفل (Background Media Session)
// ==========================================
...
function updateExternalMediaSession(title, artist, coverUrl) {
    if ('mediaSession' in navigator) {
        ...
        navigator.mediaSession.setActionHandler('play', () => {
            if (currentPlayingType === 'quran') toggleQuranPlayPause();
            else if (currentPlayingType === 'dua') toggleLibPlayPause();
        });
...
// ==========================================
// 🌟 نظام مواقيت الصلاة والأذان المفلتر (الآمن)
// ==========================================
...
function calculateNextPrayer() { 
  ...
  const elName = document.getElementById("next-prayer-name"); const elCount = document.getElementById("prayer-countdown"); 
  if (elName) elName.innerText = `الصلاة القادمة: ${nextPrayerUI.name}`; 
  if (elCount) { let timeText = `متبقي: `; if (hrs > 0) timeText += `${hrs}س و `; timeText += `${mins}د و ${secs}ث`; elCount.innerText = timeText; } 
...
// ==========================================
// 🌟 دوال مساعدة للصوت وتلوين الشريط
// ==========================================
...
function updateSliderTrackBackground(slider, color) {
    if (!slider) return;
    const min = parseFloat(slider.min) || 0; const max = parseFloat(slider.max) || 100; const val = parseFloat(slider.value) || 0;
    let percent = ((val - min) / (max - min)) * 100; if (isNaN(percent)) percent = 0;
...
// ==========================================
// 🌟 1. مشغل القرآن الكريم (الشريط العائم)
// ==========================================
...
function startQuranAudio() { 
  triggerHaptic(); stopLibAudio(); currentPlayingType = 'quran';
...
function toggleQuranPlayPause() { triggerHaptic(); if (quranAudioEl && quranAudioEl.paused) quranAudioEl.play(); else if (quranAudioEl) quranAudioEl.pause(); } 
...
// ==========================================
// 🌟 2. مكتبة الصوتيات الاحترافية (Lark Style)
// ==========================================
...
// 🌟 تحديث كلاسات الأزرار لتطابق التبويبات الجانبية (lark-tab) 🌟
function switchLibrarySubTab(subTab) {
    triggerHaptic(); currentLibSubTab = subTab;
...
function renderAudioLibrary() {
    const container = document.getElementById('audio-library-list'); if (!container) return;
    container.innerHTML = '';
    let listToRender = audioLibraryData[currentLibSubTab] || [];
    if (listToRender.length === 0) { container.innerHTML = `<div style="text-align:center; padding:30px; opacity:0.6;">القسم فارغ حالياً.</div>`; return; }

    listToRender.forEach(item => {
        const div = document.createElement('div'); div.className = "audio-track-card haptic-btn";
        div.innerHTML = `
...
function playCustomAudioFile(fileName, title, reader, cover) {
    triggerHaptic(); stopQuranAudio(); currentPlayingType = 'dua';
    if (!libAudioEl) return;
    libAudioEl.src = fileName; 
    libAudioEl.play().then(() => { updateExternalMediaSession(title, reader, cover); }).catch(e => showToast("عذراً، الملف غير متاح للتشغيل حالياً"));
...
function openFullPlayer() { triggerHaptic(); document.getElementById("full-audio-player-modal").classList.remove("hidden"); }
function closeFullPlayer() { 
    triggerHaptic(); document.getElementById("full-audio-player-modal").classList.add("hidden"); 
...
function toggleLibPlayPause() { triggerHaptic(); if (libAudioEl.paused) libAudioEl.play(); else libAudioEl.pause(); } 
function stopLibAudio() { 
    triggerHaptic(); if (libAudioEl) { libAudioEl.pause(); libAudioEl.currentTime = 0; } 
...
function seekAudio(seconds) { triggerHaptic(); if (libAudioEl) libAudioEl.currentTime += seconds; }

function updateFullPlayerUI() {
    if (libAudioEl && !libAudioEl.paused) {
        if (!fpSeekbar.isDragging) {
            fpSeekbar.max = libAudioEl.duration || 100; fpSeekbar.value = libAudioEl.currentTime;
            ...
            if(libSeekbar && !document.getElementById("floating-lib-player").classList.contains("hidden")){
                libSeekbar.max = libAudioEl.duration; libSeekbar.value = libAudioEl.currentTime;
        ...
        const floatBtn = document.getElementById("lib-btn-play"); if(floatBtn) floatBtn.innerHTML = '<i class="fa-solid fa-pause"></i>';
        const icon = document.getElementById("lib-audio-icon"); if(icon) icon.classList.add("fa-spin");
        ...
        const floatBtn = document.getElementById("lib-btn-play"); if(floatBtn) floatBtn.innerHTML = '<i class="fa-solid fa-play"></i>';
        const icon = document.getElementById("lib-audio-icon"); if(icon) icon.classList.remove("fa-spin");
    }
}
...
if (libAudioEl) {
    ...
    if(fpSeekbar){
        fpSeekbar.oninput = (e) => { fpSeekbar.isDragging = true; document.getElementById("fp-current-time").innerText = formatTime(e.target.value); };
        fpSeekbar.onchange = (e) => { fpSeekbar.isDragging = false; libAudioEl.currentTime = e.target.value; };
    }
    const miniSeekbar = document.getElementById("lib-audio-seekbar");
    if(miniSeekbar){
        miniSeekbar.oninput = (e) => { miniSeekbar.isDragging = true; document.getElementById("lib-audio-current").innerText = formatTime(e.target.value); };
        miniSeekbar.onchange = (e) => { miniSeekbar.isDragging = false; libAudioEl.currentTime = e.target.value; };
    }
}

function downloadAudioFile(event, url, title) {
    event.stopPropagation(); triggerHaptic(); showToast("بدأ تحميل " + title + "...");
    fetch(url).then(response => response.blob()).then(blob => {
        const blobUrl = window.URL.createObjectURL(blob);
        const a = document.createElement("a"); a.style.display = "none"; a.href = blobUrl; a.download = title + ".mp3";
        document.body.appendChild(a); a.click(); window.URL.revokeObjectURL(blobUrl);
        showToast("✅ تم حفظ الملف بنجاح");
    }).catch(e => { showToast("تعذر التحميل المباشر، سيتم فتح الرابط"); window.open(url, '_blank'); });
}

// ==========================================
// 🌟 نظام البث المرئي المباشر (HLS.js) 🌟
// ==========================================
let hlsInstance = null;

function playLiveChannel(url, btnElement) {
    triggerHaptic();
    ...
    if (typeof Hls !== 'undefined' && Hls.isSupported()) {
        ...
        hlsInstance.on(Hls.Events.MANIFEST_PARSED, function () {
            loader.classList.add('hidden'); video.play();
        });
        hlsInstance.on(Hls.Events.ERROR, function () {
            loader.innerHTML = "حدث خطأ في الاتصال بالبث";
        });
        ...
        loader.innerHTML = "متصفحك لا يدعم البث المباشر";
    }
}
...
// ==========================================
// 🌟 المسبحة والحاسبة والختمة والستوريات
// ==========================================
...
function executeDelete() { 
  ...
  if (deleteActionTarget === "single" && deleteTargetIndex !== null) { 
    ...
    localStorage.removeItem("financial_history_records"); renderFinancialHistory(); showToast("تم المسح بالكامل"); 
      ...
      let history = JSON.parse(localStorage.getItem('fk_history')) || []; history.splice(deleteTargetIndex, 1); localStorage.setItem('fk_history', JSON.stringify(history)); loadFkHistory(); showToast("تم مسح السجل بنجاح");
  }
  closeDeletePopup(); 
} 
...
window.renderStories = function (filterType) { triggerHaptic(); const container = document.getElementById("stories-container"); if (!container) return; container.innerHTML = ""; const tabAll = document.getElementById("tab-all-stories"); if (tabAll) tabAll.classList.remove("active"); const tabFav = document.getElementById("tab-fav-stories"); if (tabFav) tabFav.classList.remove("active"); const tabActive = document.getElementById(`tab-${filterType}-stories`); if (tabActive) tabActive.classList.add("active"); let favs = JSON.parse(localStorage.getItem("ayat_fav_stories")) || []; let storiesToRender = [...allStoriesData]; if (filterType === "fav") { storiesToRender = storiesToRender.filter((s) => favs.includes(s.url)); if (storiesToRender.length === 0) { container.innerHTML = `<div style="text-align: center; padding: 150px 20px; opacity: 0.7; font-weight: bold; color: white;">لا توجد ستوريات في المفضلة حالياً ❤️</div>`; return; } } let shuffled = [...storiesToRender]; for (let i = shuffled.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [shuffled[i], shuffled[j]] = [shuffled[j], shuffled[i]]; } shuffled.forEach((vid, index) => { const isFav = favs.includes(vid.url); const heartClass = isFav ? "fa-solid text-red" : "fa-regular"; const favActiveClass = isFav ? "active" : ""; const heartStyle = isFav ? 'style="color: #e63946;"' : ""; const srcAttr = index === 0 ? `src="${vid.url}"` : `data-src="${vid.url}"`; const preloadAttr = index === 0 ? "auto" : "none"; const autoPlayAttr = index === 0 ? "autoplay" : ""; container.innerHTML += ` <div class="story-card"> <div class="story-video-container"> <video id="story-vid-${index}" class="story-video" ${srcAttr} loop playsinline webkit-playsinline preload="${preloadAttr}" ${autoPlayAttr}></video> <div id="story-overlay-${index}" class="story-overlay-play" onclick="toggleStoryVideo(${index})"> <i class="fa-solid fa-play"></i> </div> <div class="video-progress-container"> <button id="vid-play-btn-${index}" class="vid-play-pause-btn haptic-btn" onclick="toggleStoryVideo(${index})"> <i class="fa-solid fa-play"></i> </button> <span id="vid-curr-${index}" class="video-time-text">0:00</span> <input type="range" id="vid-seek-${index}" class="video-seekbar-input" value="0" min="0" max="100" step="0.1"> <span id="vid-dur-${index}" class="video-time-text">0:00</span> </div> <div class="story-overlay-ui"> <div class="story-info-text">تلاوة عطرة</div> <div class="story-side-actions"> <button id="fav-btn-${index}" class="story-action-fav ${favActiveClass}" onclick="toggleFavorite(event, '${vid.url}', ${index})"> <i class="${heartClass} fa-heart" ${heartStyle}></i> </button> <button class="story-action-btn haptic-btn" onclick="directDownloadVideo(event, '${vid.url}')"> <i class="fa-solid fa-download"></i> <span>تنزيل</span> </button> </div> </div> </div> </div> `; }); setTimeout(() => { shuffled.forEach((vid, index) => setupVideoControls(index)); setupAutoPlayObserver(); }, 500); }; 
...
function switchFkTab(tabName) { triggerHaptic(); if ((tabName === "create" || tabName === "join") && currentFkRoomId) { showToast("أنت منضم لختمة حالياً! لا يمكنك إنشاء أو الانضمام لأخرى."); tabName = "current"; } document.querySelectorAll("#family-khatma-section .tab-btn").forEach((b) => b.classList.remove("active")); document.getElementById(`fktab-${tabName}`).classList.add("active"); document.querySelectorAll(".fk-view-panel").forEach((p) => p.classList.add("hidden")); document.getElementById(`fk-view-${tabName}`).classList.remove("hidden"); if (tabName === "current") checkCurrentRoom(); if (tabName === "history") loadFkHistory(); } 
function switchFkActiveTab(tabName) { triggerHaptic(); document.getElementById("fk-subtab-mine").classList.remove("active"); document.getElementById("fk-subtab-all").classList.remove("active"); document.getElementById(`fk-subtab-${tabName}`).classList.add("active"); if (tabName === "mine") { document.getElementById("fk-view-mine").classList.remove("hidden"); document.getElementById("fk-view-all").classList.add("hidden"); } else { document.getElementById("fk-view-mine").classList.add("hidden"); document.getElementById("fk-view-all").classList.remove("hidden"); } } 
function generateFkRoomCode() { return Math.floor(100000 + Math.random() * 900000).toString(); } 
function createFamilyRoom() { triggerHaptic(); if (currentFkRoomId) return showToast("أنت في غرفة حالياً!"); const name = document.getElementById("fk-create-name").value.trim(); const khatmaName = document.getElementById("fk-create-khatma-name").value.trim(); const type = document.getElementById("fk-create-type").value; if (!name) return showToast("يرجى إدخال اسمك"); if (!database) return showToast("لا يوجد اتصال بالسيرفر"); const roomId = generateFkRoomCode(); const finalKhatmaName = khatmaName || "ختمة عائلية مشتركة"; const roomData = { code: roomId, creator: fkMyUid, khatmaName: finalKhatmaName, type: type, status: "waiting", participants: {} }; roomData.participants[fkMyUid] = { name: name }; database.ref("family_khatmas/" + roomId).set(roomData).then(() => { localStorage.setItem("fk_my_name", name); localStorage.setItem("fk_current_room", roomId); currentFkRoomId = roomId; showToast("تم إنشاء الغرفة بنجاح!"); switchFkTab("current"); }).catch((e) => { showToast("حدث خطأ في الاتصال، حاول مجدداً"); }); } 
function joinFamilyRoom() { triggerHaptic(); if (currentFkRoomId) return showToast("أنت في غرفة حالياً!"); const name = document.getElementById("fk-join-name").value.trim(); const code = document.getElementById("fk-join-code").value.trim(); if (!name || !code) return showToast("يرجى إدخال الاسم وكود الغرفة"); if (!database) return; database.ref("family_khatmas/" + code).once("value").then((snapshot) => { if (!snapshot.exists()) return showToast("كود الغرفة غير صحيح!"); const room = snapshot.val(); if (room.status !== "waiting") return showToast("عذراً، الختمة بدأت بالفعل وتم توزيعها!"); database.ref("family_khatmas/" + code + "/participants/" + fkMyUid).set({ name: name }).then(() => { localStorage.setItem("fk_my_name", name); localStorage.setItem("fk_current_room", code); currentFkRoomId = code; showToast("تم الانضمام بنجاح!"); switchFkTab("current"); }); }); } 
function checkCurrentRoom() { if (!currentFkRoomId) { document.getElementById("fk-no-room-msg").classList.remove("hidden"); document.getElementById("fk-active-room-container").classList.add("hidden"); return; } document.getElementById("fk-no-room-msg").classList.add("hidden"); document.getElementById("fk-active-room-container").classList.remove("hidden"); document.getElementById("fk-room-code-display").innerText = currentFkRoomId; if (fkRoomListener) database.ref("family_khatmas/" + currentFkRoomId).off("value", fkRoomListener); fkRoomListener = database.ref("family_khatmas/" + currentFkRoomId).on("value", (snapshot) => { const room = snapshot.val(); if (!room) { currentFkRoomId = null; localStorage.removeItem("fk_current_room"); checkCurrentRoom(); return; } const kNameDisplay = document.getElementById("fk-khatma-name-display"); if (kNameDisplay) kNameDisplay.innerText = room.khatmaName || "ختمة عائلية"; if (room.status === "finished") { saveToFkHistory(room); currentFkRoomId = null; localStorage.removeItem("fk_current_room"); showToast("🎉 مبارك! تمت الختمة العائلية بنجاح وتقبل الله أعمالكم."); setTimeout(() => switchFkTab("history"), 2000); return; } const statusBadge = document.getElementById("fk-room-status-badge"); const distBtn = document.getElementById("fk-start-distribution-btn"); const distContainer = document.getElementById("fk-distribution-container"); const instructionNote = document.getElementById("fk-instruction-note"); const waitingContainer = document.getElementById("fk-waiting-container"); if (room.status === "waiting") { statusBadge.innerText = "قيد الإنشاء (بانتظار اكتمال العدد)"; statusBadge.style.background = "var(--accent)"; instructionNote.classList.add("hidden"); distContainer.classList.add("hidden"); waitingContainer.classList.remove("hidden"); if (room.creator === fkMyUid) distBtn.classList.remove("hidden"); else distBtn.classList.add("hidden"); } else if (room.status === "active") { statusBadge.innerText = "الختمة جارية الآن"; statusBadge.style.background = "#2ecc71"; distBtn.classList.add("hidden"); waitingContainer.classList.add("hidden"); instructionNote.classList.remove("hidden"); distContainer.classList.remove("hidden"); renderPortionsGrid(room); } const pList = document.getElementById("fk-participants-list"); pList.innerHTML = ""; if (room.participants) { Object.values(room.participants).forEach((p) => { pList.innerHTML += `<div class="participant-badge"><i class="fa-solid fa-user"></i> ${p.name}</div>`; }); } }); } 
function promptLeaveFkRoom() { triggerHaptic(); deleteActionTarget = "leave_fk_room"; document.getElementById("delete-popup-msg").innerText = "هل أنت متأكد من الخروج من الختمة الحالية؟"; document.getElementById("delete-confirm-popup").classList.remove("hidden-popup"); } 
function shareRoomCode() { triggerHaptic(); if (currentFkRoomId) { const text = `انضم إلينا في الختمة العائلية على تطبيق آيات!\nكود الغرفة هو: ${currentFkRoomId}`; if (typeof window.AndroidBridge !== "undefined" && typeof window.AndroidBridge.shareApp === "function") { window.AndroidBridge.shareApp(text); } else if (navigator.share) { navigator.share({ title: "الختمة العائلية", text: text }).catch((e) => {}); } else { navigator.clipboard.writeText(currentFkRoomId).then(() => { showToast("تم نسخ كود الغرفة للمشاركة"); }); } } } 
function startFamilyDistribution() { triggerHaptic(); if (!database || !currentFkRoomId) return; database.ref("family_khatmas/" + currentFkRoomId).once("value").then((snapshot) => { const room = snapshot.val(); const pKeys = Object.keys(room.participants); if (pKeys.length < 1) return showToast("لا يوجد مشاركون!"); const totalItems = room.type === "ajza" ? 30 : 114; let portions = {}; let pIndex = 0; for (let i = 1; i <= totalItems; i++) { let uid = pKeys[pIndex]; portions[i] = { id: i, assignedToUid: uid, assignedToName: room.participants[uid].name, status: "pending" }; pIndex++; if (pIndex >= pKeys.length) pIndex = 0; } database.ref("family_khatmas/" + currentFkRoomId).update({ status: "active", portions: portions }).then(() => showToast("تم التوزيع.. بسم الله نبدأ!")); }); } 
function renderPortionsGrid(room) { const gridMine = document.getElementById("fk-portions-grid-mine"); const listAll = document.getElementById("fk-participants-progress-list"); gridMine.innerHTML = ""; listAll.innerHTML = ""; const totalItems = room.type === "ajza" ? 30 : 114; let allCompleted = true; let myPortionsCount = 0; let userStats = {}; Object.keys(room.participants).forEach((uid) => { userStats[uid] = { name: room.participants[uid].name, total: 0, completed: 0, portions: [] }; }); for (let i = 1; i <= totalItems; i++) { if (!room.portions || !room.portions[i]) { allCompleted = false; continue; } const portion = room.portions[i]; if (portion.status !== "completed") allCompleted = false; let portionTrueName = getPortionTrueName(room.type, i); if (userStats[portion.assignedToUid]) { userStats[portion.assignedToUid].total++; if (portion.status === "completed") userStats[portion.assignedToUid].completed++; userStats[portion.assignedToUid].portions.push({ name: portionTrueName, status: portion.status }); } if (portion.assignedToUid === fkMyUid) { myPortionsCount++; let cardClass = portion.status === "completed" ? "portion-completed" : "portion-mine haptic-btn"; let badgeClass = portion.status === "completed" ? "completed-badge" : "mine-badge"; let badgeText = portion.status === "completed" ? "تمت القراءة <i class='fa-solid fa-check'></i>" : "لم يُقرأ"; let clickAction = portion.status === "completed" ? "" : `onclick="markPortionDone(${i})"`; gridMine.innerHTML += ` <div class="portion-card ${cardClass}" ${clickAction}> <div class="portion-title" style="font-size:1.1rem; font-family:'Amiri';">${portionTrueName}</div> <div class="portion-status-badge ${badgeClass}">${badgeText}</div> </div> `; } } if (myPortionsCount === 0) { gridMine.innerHTML = `<div style="text-align:center; grid-column:1/-1; opacity:0.7; padding: 20px;">لا توجد حصص مخصصة لك في هذا التوزيع.</div>`; } Object.values(userStats).forEach((stat) => { let p = stat.total > 0 ? Math.round((stat.completed / stat.total) * 100) : 0; let chipsHtml = ""; stat.portions.forEach((portion) => { let chipClass = portion.status === "completed" ? "pp-chip done" : "pp-chip"; let icon = portion.status === "completed" ? '<i class="fa-solid fa-check-double"></i> ' : '<i class="fa-solid fa-book-open"></i> '; chipsHtml += `<div class="${chipClass}">${icon}${portion.name}</div>`; }); if (stat.total > 0) { listAll.innerHTML += ` <div class="participant-progress-card"> <div class="pp-header"> <div class="pp-name"><i class="fa-solid fa-user-pen"></i> ${stat.name}</div> <div class="pp-stats">أكمل ${stat.completed} من ${stat.total}</div> </div> <div class="progress-bar-container" style="height: 8px; margin-bottom: 15px; background: rgba(0,0,0,0.05);"> <div class="progress-bar-fill" style="width:${p}%; border-radius: 10px;"></div> </div> <div style="font-size: 0.85rem; font-weight: bold; color: var(--text); opacity: 0.8; margin-bottom: 12px;"> الحصص المخصصة: </div> <div class="pp-portions-container"> ${chipsHtml} </div> </div> `; } }); if (allCompleted) { database.ref("family_khatmas/" + currentFkRoomId).update({ status: "finished" }); } } 
function markPortionDone(portionId) { triggerHaptic(); if (!database || !currentFkRoomId) return; database.ref(`family_khatmas/${currentFkRoomId}/portions/${portionId}`).update({ status: "completed" }).then(() => showToast("تقبل الله طاعتك!")); } 
