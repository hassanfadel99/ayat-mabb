 ...
 * النسخة النهائية - شاملة إصلاح التحديثات والقبلة والختمة
 */ 

// 🌟 قمنا بتثبيت رقم النسخة هنا لضمان استقبال أي تحديث من فايربيس (مثل 5.1) 🌟
...
function checkForUpdates() {
  ...
  database.ref('app_settings').once('value').then((snapshot) => {
    ...
    if (data && data.latest_version) {
       let firebaseVersion = parseFloat(data.latest_version);
       if (firebaseVersion > APP_CURRENT_VERSION) {
...
function loadPreferences() { changeLayout(localStorage.getItem("ayat_layout") || "compact"); if (localStorage.getItem("ayat_darkmode") === "true") setAppMode("night"); else setAppMode("day"); } 
...
window.addEventListener("DOMContentLoaded", () => { 
    ...
    try { checkForUpdates(); } catch (e) {} // التحديث يعمل الآن بنجاح
