 ...
 * النسخة المحدثة (4.2) - نظام القبلة الناعم والإشعارات اللحظية
 */ 

let APP_CURRENT_VERSION = 4.2; 
...
const i18n = { 
  ar: { 
    ...
    azkar: "حصن المسلم", azkar_desc: "موسوعة الأذكار", 
...
function openGPSSetting() {
    ...
    // إخبار التطبيق أننا ننتظر عودة المستخدم من الإعدادات
...
// 🌟 مراقبة عودة المستخدم للتطبيق لعمل فحص تلقائي 🌟
document.addEventListener("visibilitychange", function() {
    if (document.visibilityState === 'visible' && isWaitingForGPS) {
        isWaitingForGPS = false;
        document.getElementById('gps-prompt-popup').classList.add('hidden-popup');
        initQibla(); // إعادة المحاولة تلقائياً بمجرد رجوعه للتطبيق
    }
});
...
let currentDisplayedHeading = 0; // متغير للتنعيم (إزالة الرعشة)
...
function handleOrientation(event) {
    ...
    // استخراج الزاوية للأيفون والأندرويد
    ...
    if (!isValid) return; // الهاتف لا يرسل بيانات (تجميد)
    ...
    if (disc) {
        ...
        // 🌟 خوارزمية إيجاد أقصر طريق للدوران (تمنع الدوران العكسي السريع والارتداد)
        let diff = targetHeading - currentDisplayedHeading;
        diff = ((diff + 180) % 360) - 180; 
        
        // 🌟 خوارزمية التنعيم (Low Pass Filter) لإزالة الرعشة
