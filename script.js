 ...
 * النسخة المحدثة (3.8) - إشعارات الختمة والتعامل مع إغلاق الـ GPS
 */ 
...
function openGPSSetting() {
    ...
    if (typeof AndroidBridge !== "undefined" && typeof AndroidBridge.openLocationSettings === "function") {
...
function checkCurrentRoom() {
    ...
    fkRoomListener = database.ref('family_khatmas/' + currentFkRoomId).on('value', snapshot => {
        ...
        if (typeof AndroidBridge !== "undefined" && typeof AndroidBridge.sendLocalNotification === "function") {
            ...
            if (room.creator === fkMyUid && room.participants) {
                ...
                if (previousParticipantsCount !== -1 && currentCount > previousParticipantsCount) {
                    let newParticipantName = "شخص جديد";
                    let keys = Object.keys(room.participants);
                    newParticipantName = room.participants[keys[keys.length - 1]].name;
