...
function triggerAdhanAlert(prayerName) { 
  ...
  if (typeof quranAudioEl !== "undefined" && quranAudioEl && !quranAudioEl.paused) { stopAudio(); } 
...
// ==========================================
// 🌟 نظام مكتبة الأدعية والأذكار الصوتية 🌟
// ==========================================

// تم مسح المجلدات من المسارات، الآن التطبيق سيقرأ الملفات المرفوعة بجوار index مباشرة
...
let currentLibMainTab = 'azkar'; 
let currentLibSubTab_azkar = 'sabah';
let currentLibSubTab_duaa = 'duaa_days';
let audioPlayerObject = new Audio();
let currentAudioType = null;
let isPlayingCustomAudio = false; 

function shuffleAudioArray(array) {
    let shuffled = [...array];
...
window.toggleAudioAzkar = function(type) {
    triggerHaptic();
    const btnSabahIcon = document.querySelector('#btn-sabah-audio .block-icon');
    const btnMassaIcon = document.querySelector('#btn-massa-audio .block-icon');

    if(btnSabahIcon) btnSabahIcon.innerHTML = '<i class="fa-solid fa-sun"></i>';
    if(btnMassaIcon) btnMassaIcon.innerHTML = '<i class="fa-solid fa-moon"></i>';

    if (currentAudioType === type && !audioPlayerObject.paused) {
    ...
    if (!trackList || trackList.length === 0) {
        showToast("لم يتم إضافة ملفات في هذا القسم بعد!");
        return;
    }

    // هنا نختار الملف الخاص بالصباح أو المساء (بدل العشوائي القديم)
    let selectedTrack;
    if (type === 'sabah') {
        selectedTrack = trackList.find(t => t.fileName === "sabah.mp3");
    } else {
        selectedTrack = trackList.find(t => t.fileName === "massa.mp3");
    }

    if (!selectedTrack) {
         showToast("ملف الصوت غير موجود.");
         return;
    }

    audioPlayerObject.pause();
    audioPlayerObject.src = selectedTrack.fileName; 
    
    audioPlayerObject.play().then(() => {
        currentAudioType = type;
        showToast(type === 'sabah' ? "جاري تشغيل أذكار الصباح..." : "جاري تشغيل أذكار المساء...");
        
        if (type === 'sabah' && btnSabahIcon) {
            btnSabahIcon.innerHTML = '<i class="fa-solid fa-circle-stop" style="color:#e63946;"></i>';
        } else if (type === 'massa' && btnMassaIcon) {
            btnMassaIcon.innerHTML = '<i class="fa-solid fa-circle-stop" style="color:#e63946;"></i>';
        }
    }).catch(e => {
        showToast("تنبيه: تأكد من رفع ملف " + selectedTrack.fileName + " في جيت هب بشكل صحيح");
    });
};

audioPlayerObject.onended = () => {
    currentAudioType = null;
    const btnSabahIcon = document.querySelector('#btn-sabah-audio .block-icon');
    const btnMassaIcon = document.querySelector('#btn-massa-audio .block-icon');
    if(btnSabahIcon) btnSabahIcon.innerHTML = '<i class="fa-solid fa-sun"></i>';
...
function switchLibraryMainTab(tab) {
    triggerHaptic();
    currentLibMainTab = tab;
    
    document.getElementById('lib-tab-azkar').classList.remove('active');
    document.getElementById('lib-tab-duaa').classList.remove('active');
    document.getElementById(`lib-tab-${tab}`).classList.add('active');

    if (tab === 'azkar') {
        document.getElementById('lib-subtabs-azkar').classList.remove('hidden');
        document.getElementById('lib-subtabs-duaa').classList.add('hidden');
    } else {
        document.getElementById('lib-subtabs-azkar').classList.add('hidden');
        document.getElementById('lib-subtabs-duaa').classList.remove('hidden');
    }
    renderAudioLibrary();
}
...
function switchLibrarySubTab(subTab) {
    ...
    document.querySelectorAll('#lib-subtabs-duaa .tab-btn').forEach(btn => btn.classList.remove('active'));
...
function renderAudioLibrary() {
    ...
    listToRender.forEach(item => {
        ...
        div.onclick = () => {
            playCustomAudioFile(item.fileName, item.title);
        };
...
function playCustomAudioFile(fileName, title) {
    triggerHaptic();
    isPlayingCustomAudio = true; 

    if (!audioPlayerObject.paused) {
        audioPlayerObject.pause();
        currentAudioType = null;
        const btnSabahIcon = document.querySelector('#btn-sabah-audio .block-icon');
        const btnMassaIcon = document.querySelector('#btn-massa-audio .block-icon');
    ...
    const surahEl = document.getElementById("f-audio-surah");
    const readerEl = document.getElementById("f-audio-reader");
    
    if (surahEl) surahEl.innerText = title;
    if (readerEl) readerEl.innerText = "الأدعية والزيارات";
}

// ==========================================
// 🌟 القواميس، المسبحة، والخدمات العادية 
// ==========================================
const i18n = { 
  ar: { 
    ...
    azkar: "حصن المسلم", azkar_desc: "موسوعة الأذكار", misbaha: "المسبحة الذكية", misbaha_desc: "عداد الاستغفار", 
    financial: "الحاسبة الشرعية", financial_desc: "الزكاة والخمس", settings_short: "التخصيص", settings_desc: "الواجهة واللغات", 
    khatma: "الختمة الشخصية", khatma_desc: "جدول القراءة الخاص", family_khatma: "الختمة العائلية", family_khatma_desc: "ختمة مشتركة مع العائلة", 
    ...
    tap_to_count: "انقر للتسبيح", reset: "تصفير", zakat: "الزكاة", khums: "الخمس", records: "السجل", 
    zakat_title: "حاسبة الزكاة (2.5%)", total_money: "رأس المال المتوفر:", zakat_due: "الزكاة المستحقة:", save_record: "حفظ", 
    khums_title: "حاسبة الخمس (20%)", surplus_money: "الأموال الزائدة:", khums_due: "الخمس الواجب:", imam_share: "سهم الإمام:", 
    sadah_share: "سهم السادة:", history_title: "السجل المالي", khatma_duration: "أدخل المدة المستهدفة (بالأيام):", 
    khatma_daily: "المطلوب يومياً:", khatma_start: "اعتماد", ward_goal_title: "قراءة القرآن اليومية", 
    ward_goal_desc: "تحديد عدد الصفحات المطلوب قراءتها يومياً", lang_title: "لغة التطبيق", color_wheel_desc: "اسحب الشريط لتغيير لون الواجهة", 
    pick_color: "اختر لون مخصص", template_theme: "تخصيص قالب المظهر", manual_theme: "تخصيص لون الواجهة", layout_title: "شكل وكثافة الواجهة", 
...
function checkCurrentRoom() { 
  if (!currentFkRoomId) { 
    document.getElementById("fk-no-room-msg").classList.remove("hidden"); document.getElementById("fk-active-room-container").classList.add("hidden"); 
    return; 
  } 
...
function loadFkHistory() { 
  const list = document.getElementById("fk-history-list"); list.innerHTML = ""; 
  let history = JSON.parse(localStorage.getItem("fk_history")) || []; 
