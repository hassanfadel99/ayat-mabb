/** 
 * تطبيق آيات - الملف البرمجي الشامل (النسخة 3.4)
 * يتضمن التحديثات الخاصة بتشغيل الصوت في الخلفية عبر Android Service
 */ 

let APP_CURRENT_VERSION = 3.4; 
...
// ==========================================
// 🌟 6. نظام إشعارات شاشة القفل (Media Session)
// ==========================================
...
function calculateNextPrayer() { 
    ...
    if (elName) {
        elName.innerText = `الصلاة القادمة: ${nextPrayerUI.name}`; 
    }
...
window.playDynamicLiveChannel = function(channelKey, defaultUrl, btnElement) {
    ...
    if (note) note.style.display = "block"; // إظهار رسالة التنبيه
    
    // 🌟 البحث في فايربيس أولاً عن رابط جديد
    if (database) {
...
// ==========================================
// 🌟 9. مشغل القرآن الكريم (الشريط العائم وتفعيل الخلفية)
// ==========================================
...
function updateQuranSeekbarSmoothly() { 
    if (quranAudioEl && !quranAudioEl.paused) { 
        if (quranSeekbar && !quranSeekbar.isDragging) { 
            ...
            if (quranCurrentTimeEl) {
                quranCurrentTimeEl.innerText = formatTime(quranAudioEl.currentTime); 
            }
...
if (quranAudioEl) { 
    quranAudioEl.onloadedmetadata = () => { 
        ...
        if (quranDurationEl) {
            quranDurationEl.innerText = formatTime(quranAudioEl.duration); 
        }
    }; 
    ...
    if (quranSeekbar) { 
        ...
        quranSeekbar.oninput = () => { 
            if (quranCurrentTimeEl) {
                quranCurrentTimeEl.innerText = formatTime(quranSeekbar.value); 
            }
            updateSliderTrackBackground(quranSeekbar, quranTrackColor); 
        }; 
    } 
...
window.stopQuranAudio = function() { 
    ...
    // 🌟 إيقاف الخدمة للسماح للهاتف بالراحة 🌟
...
// ==========================================
// 🌟 10. مكتبة الأدعية والمشغل الكامل (وتفعيل الخلفية)
// ==========================================
...
window.switchLibrarySubTab = function(subTab) { 
    ...
    document.querySelectorAll('.lark-tab').forEach(btn => {
        btn.classList.remove('active');
    }); 
...
window.playCustomAudioFile = function(fileName, title, reader, cover) { 
    ...
    libAudioEl.play().then(() => { 
        ...
        // 🌟 الأمر السحري لبدء خدمة الخلفية للأدعية 🌟
        if (typeof window.AndroidBridge !== "undefined" && typeof window.AndroidBridge.startBackgroundAudio === "function") {
            window.AndroidBridge.startBackgroundAudio();
        }
    }).catch(e => {
        showToast("عذراً، الملف غير متاح للتشغيل حالياً");
    }); 
...
window.seekAudio = function(seconds) { 
    triggerHaptic(); 
    if (libAudioEl) {
        libAudioEl.currentTime += seconds; 
    }
} 
...
function updateFullPlayerUI() { 
    if (libAudioEl && !libAudioEl.paused) { 
        ...
        if(floatBtn) {
            floatBtn.innerHTML = '<i class="fa-solid fa-pause"></i>'; 
        }
        ...
        const floatBtn = document.getElementById("lib-btn-play"); 
        if(floatBtn) {
            floatBtn.innerHTML = '<i class="fa-solid fa-play"></i>'; 
        }
...
async function loadSurahs() { 
    try { 
        ...
        if (c) {
            c.innerHTML = "<div style='grid-column: 1/-1; text-align:center; padding:20px;'>تعذر التحميل، يرجى التحقق من الإنترنت.</div>"; 
        }
    } 
} 
...
window.changeFontSize = function(amt) { 
    ...
    document.querySelectorAll(".ayah-box").forEach((s) => {
        s.style.fontSize = defaultFontSize + "px";
    }); 
} 

function saveReadingProgress(num, name, idx) { 
    localStorage.setItem("lastQuranProgress", JSON.stringify({ 
        surahNum: num, 
...
function updateWardDashboardProgress() { 
    ...
    if (svd && resBox) { 
        ...
        if (resTxt) {
            resTxt.innerHTML = `سورة ${cleanSurahName(pr.surahName)} - صفحة ${pr.pageIndex + 1}`; 
        }
...
window.activateKhatmaChallenge = function() { 
    ...
    localStorage.setItem("active_khatma_challenge", JSON.stringify({ 
        days: d, 
        pagesPerDay: Math.ceil(604 / d) 
    })); 
...
window.renderStories = function (filterType) { 
        ...
        container.innerHTML += ` 
            <div class="story-card"> 
                <div class="story-video-container"> 
                    <video id="story-vid-${index}" class="story-video" ${srcAttr} loop playsinline webkit-playsinline preload="${preloadAttr}" ${autoPlayAttr}></video> 
    ...
    setTimeout(() => { 
        shuffled.forEach((vid, index) => setupVideoControls(index)); 
...
window.switchFkTab = function(tabName) { 
    ...
    document.querySelectorAll("#family-khatma-section .tab-btn").forEach((b) => {
        b.classList.remove("active");
    }); 
    
    document.getElementById(`fktab-${tabName}`).classList.add("active"); 
    
    document.querySelectorAll(".fk-view-panel").forEach((p) => {
...
function checkCurrentRoom() { 
    ...
    if (fkRoomListener) {
        database.ref("family_khatmas/" + currentFkRoomId).off("value", fkRoomListener); 
    }
    ...
    fkRoomListener = database.ref("family_khatmas/" + currentFkRoomId).on("value", (snapshot) => { 
        ...
        if (room.status === "waiting") { 
            ...
            if (room.creator === fkMyUid) {
                distBtn.classList.remove("hidden"); 
            } else {
    ...
    for (let i = 1; i <= totalItems; i++) { 
        ...
        if (userStats[portion.assignedToUid]) { 
            ...
            if (portion.status === "completed") {
                userStats[portion.assignedToUid].completed++; 
            }
...
// ==========================================
// 🌟 تحميل مبدئي للبيانات عند بدء التطبيق
// ==========================================
