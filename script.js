/** 
 * تطبيق آيات - الملف البرمجي الشامل (النسخة 3.5)
 * يحتوي على ربط ذكي بالكامل مع مشغل نظام الأندرويد
 */ 

let APP_CURRENT_VERSION = 3.5; 
...
// ==========================================
// 🌟 3. تشغيل التطبيق والتنقل الأساسي
// ==========================================
...
// ==========================================
// 🌟 6. نظام الإشعارات العكسية (من الأندرويد للويب) 🌟
// ==========================================
let currentPlayingType = null;

// هذه الدالة سيستدعيها الأندرويد عندما يضغط المستخدم على "إيقاف مؤقت" من الإشعار
window.androidPauseAudio = function() {
    if (quranAudioEl && !quranAudioEl.paused) {
        window.toggleQuranPlayPause();
    } else if (libAudioEl && !libAudioEl.paused) {
...
function updateExternalMediaSession(title, artist, coverUrl) {
    if ('mediaSession' in navigator) {
        navigator.mediaSession.metadata = new MediaMetadata({
            ...
            album: 'تطبيق آيات',
            artwork: [
                { src: coverUrl || 'https://i.ibb.co/yFY29WhK/image.jpg', sizes: '512x512', type: 'image/jpeg' }
            ]
        });
        
        navigator.mediaSession.setActionHandler('play', () => {
            if (currentPlayingType === 'quran' && quranAudioEl) { 
                quranAudioEl.play(); 
            } else if (currentPlayingType === 'dua' && libAudioEl) { 
                libAudioEl.play(); 
            }
        });
...
// ==========================================
// 🌟 8. البث المباشر (الديناميكي عبر فايربيس)
// ==========================================
...
// ==========================================
// 🌟 9. مشغل القرآن الكريم (مع تفعيل إشعار الأندرويد) 🌟
// ==========================================
...
window.startQuranAudio = function() { 
    ...
    quranAudioEl.play().then(() => {
        ...
        // إرسال الإسم للقارئ والاسم للأندرويد
...
// ==========================================
// 🌟 10. مكتبة الأدعية والمشغل الكامل (مع إشعار الميديا) 🌟
// ==========================================
...
function updateWardDashboardProgress() { 
    ...
    const wGoal = document.getElementById("setting-ward-goal"); if (wGoal) wGoal.value = g; 
...
// ==========================================
// 🌟 12. المسبحة والحاسبة والختمة الشخصية
// ==========================================
...
window.calculateZakat = function() { 
    const inp = document.getElementById("zakat-amount-input"); if (!inp) return; 
    const a = parseFloat(inp.value); if (isNaN(a) || a <= 0) return; 
...
window.calculateKhums = function() { 
    const inp = document.getElementById("khums-amount-input"); if (!inp) return; 
    const a = parseFloat(inp.value); if (isNaN(a) || a <= 0) return; 
    const k = a * 0.2; const kOut = document.getElementById("khums-total-output"); if (kOut) kOut.innerText = k.toFixed(2); 
    const kImam = document.getElementById("khums-imam-output"); if (kImam) kImam.innerText = (k / 2).toFixed(2); 
    const kSad = document.getElementById("khums-sadah-output"); if (kSad) kSad.innerText = (k / 2).toFixed(2); 
} 
...
window.executeDelete = function() { 
    triggerHaptic(); 
    if (deleteActionTarget === "single" && deleteTargetIndex !== null) { const h = JSON.parse(localStorage.getItem("financial_history_records")) || []; h.splice(deleteTargetIndex, 1); localStorage.setItem("financial_history_records", JSON.stringify(h)); renderFinancialHistory(); showToast("تم حذف السجل"); } 
    else if (deleteActionTarget === "all") { localStorage.removeItem("financial_history_records"); renderFinancialHistory(); showToast("تم المسح بالكامل"); } 
    else if (deleteActionTarget === "leave_fk_room") { if (currentFkRoomId && database) { database.ref('family_khatmas/' + currentFkRoomId).once('value').then(snapshot => { const room = snapshot.val(); if (room && room.creator === fkMyUid) { database.ref('family_khatmas/' + currentFkRoomId).remove(); } else if (room) { database.ref('family_khatmas/' + currentFkRoomId + '/participants/' + fkMyUid).remove(); } currentFkRoomId = null; localStorage.removeItem('fk_current_room'); showToast("تم الخروج من الختمة بنجاح"); switchFkTab('create'); }); } } 
    else if (deleteActionTarget === "single_fk_history" && deleteTargetIndex !== null) { let history = JSON.parse(localStorage.getItem('fk_history')) || []; history.splice(deleteTargetIndex, 1); localStorage.setItem('fk_history', JSON.stringify(history)); loadFkHistory(); showToast("تم مسح السجل بنجاح"); } 
    closeDeletePopup(); 
} 
...
function setupStoriesListener() { 
    ...
    database.ref("stories").on("value", (snapshot) => { allStoriesData = []; const data = snapshot.val(); if (data) { for (let key in data) { allStoriesData.push({ id: key, url: data[key].url }); } } }); 
} 

window.renderStories = function (filterType) { 
    triggerHaptic(); const container = document.getElementById("stories-container"); if (!container) return; 
    container.innerHTML = ""; const tabAll = document.getElementById("tab-all-stories"); if (tabAll) tabAll.classList.remove("active"); const tabFav = document.getElementById("tab-fav-stories"); if (tabFav) tabFav.classList.remove("active"); const tabActive = document.getElementById(`tab-${filterType}-stories`); if (tabActive) tabActive.classList.add("active"); 
    let favs = JSON.parse(localStorage.getItem("ayat_fav_stories")) || []; let storiesToRender = [...allStoriesData]; 
    if (filterType === "fav") { storiesToRender = storiesToRender.filter((s) => favs.includes(s.url)); if (storiesToRender.length === 0) { container.innerHTML = `<div style="text-align: center; padding: 150px 20px; opacity: 0.7; font-weight: bold; color: white;">لا توجد ستوريات في المفضلة حالياً ❤️</div>`; return; } } 
    let shuffled = [...storiesToRender]; for (let i = shuffled.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [shuffled[i], shuffled[j]] = [shuffled[j], shuffled[i]]; } 
...
window.toggleFavorite = function (event, url, index) { 
    ...
    try { let favs = JSON.parse(localStorage.getItem("ayat_fav_stories")) || []; const btn = document.getElementById(`fav-btn-${index}`); if (!btn) return; 
        ...
        localStorage.setItem("ayat_fav_stories", JSON.stringify(favs)); } catch (e) { console.error(e); } 
}; 
...
window.toggleStoryVideo = function (index) { 
    triggerHaptic(); const video = document.getElementById(`story-vid-${index}`); const overlay = document.getElementById(`story-overlay-${index}`); const miniPlayBtn = document.getElementById(`vid-play-btn-${index}`); 
    if (!video) return; 
    ...
    else { video.pause(); if (overlay) overlay.classList.remove("playing"); if (miniPlayBtn) miniPlayBtn.innerHTML = '<i class="fa-solid fa-play"></i>'; currentPlayingVideo = null; } 
}; 

function setupAutoPlayObserver() { 
    const videos = document.querySelectorAll(".story-video"); 
    if ("IntersectionObserver" in window) { const observer = new IntersectionObserver((entries) => { entries.forEach((entry) => { const vid = entry.target; const overlay = document.getElementById(vid.id.replace("story-vid", "story-overlay")); const miniPlayBtn = document.getElementById(vid.id.replace("story-vid-", "vid-play-btn-")); 
        if (entry.isIntersecting) { if (!vid.getAttribute("src") && vid.getAttribute("data-src")) { vid.setAttribute("src", vid.getAttribute("data-src")); vid.load(); } vid.play().then(() => { if (overlay) overlay.classList.add("playing"); if (miniPlayBtn) miniPlayBtn.innerHTML = '<i class="fa-solid fa-pause"></i>'; currentPlayingVideo = vid; const currentIdNum = parseInt(vid.id.replace("story-vid-", "")); const nextVid = document.getElementById(`story-vid-${currentIdNum + 1}`); if (nextVid && !nextVid.getAttribute("src") && nextVid.getAttribute("data-src")) { nextVid.setAttribute("src", nextVid.getAttribute("data-src")); nextVid.setAttribute("preload", "metadata"); } }).catch((e) => { vid.pause(); if (overlay) overlay.classList.remove("playing"); if (miniPlayBtn) miniPlayBtn.innerHTML = '<i class="fa-solid fa-play"></i>'; }); } 
        else { vid.pause(); vid.currentTime = 0; if (overlay) overlay.classList.remove("playing"); if (miniPlayBtn) miniPlayBtn.innerHTML = '<i class="fa-solid fa-play"></i>'; if (vid.getAttribute("src")) { vid.removeAttribute("src"); vid.load(); } } }); }, { threshold: 0.5 }); videos.forEach((vid) => observer.observe(vid)); } 
} 
...
window.directDownloadVideo = async function (event, url) { 
    ...
    try { const response = await fetch(url, { signal }); const contentLength = response.headers.get("content-length"); const total = parseInt(contentLength, 10); let loaded = 0; const reader = response.body.getReader(); const chunks = []; 
    ...
    catch (error) { if (error.name === "AbortError") { console.log("Download cancelled"); } else { if (overlay) overlay.classList.add("hidden-popup"); window.open(url, "_blank"); } } 
}; 
...
window.switchFkActiveTab = function(tabName) { 
    triggerHaptic(); 
    document.getElementById("fk-subtab-mine").classList.remove("active"); document.getElementById("fk-subtab-all").classList.remove("active"); document.getElementById(`fk-subtab-${tabName}`).classList.add("active"); 
    if (tabName === "mine") { document.getElementById("fk-view-mine").classList.remove("hidden"); document.getElementById("fk-view-all").classList.add("hidden"); } 
    else { document.getElementById("fk-view-mine").classList.add("hidden"); document.getElementById("fk-view-all").classList.remove("hidden"); } 
} 
