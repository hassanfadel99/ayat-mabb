/**
 * تطبيق آيات - الملف البرمجي الكامل (script.js)
 */

// 🌟 قراءة رقم الإصدار ديناميكياً
...
function checkForUpdates() {
  // عدم إظهار التحديث في المتصفح العادي (نسخة الويب)
...
const i18n = {
  ...
  en: {
    ...
    travel: "Travel",
    prayer: "Prayer",
    asma_hero_desc: "Tap on any name to learn its meaning and mention in the Quran.",
    ...
    about_desc: "Ayat App is your digital companion for an integrated faith journey.",
    copy: "Copy Text",
    close: "Close",
    share: "Share App",
  ...
  ku: {
    ...
    travel: "سەفەر",
    prayer: "نوێژ",
    asma_hero_desc: "پەنجە بنێ بە هەر ناوێکدا بۆ زانینی مانا و شوێنی لە قورئاندا.",
    ...
    about_desc: "پڕۆگرامی ئايات هاوڕێی دیجیتاڵیی تۆیە بۆ گەشتێکی ئیمانی.",
    copy: "لەبەرگرتنەوە",
    close: "داخستن",
    share: "بڵاوکردنەوە",
  ...
  fa: {
    ...
    about_desc: "برنامه آیات همراه دیجیتال شما در یک سفر ایمانی است.",
    copy: "کپی متن",
    close: "بستن",
    share: "اشتراک گذاری",
...
function openAsmaPopup(asma) {
  triggerHaptic();
  const n = document.getElementById("popup-asma-name");
  if (n) n.innerText = asma.name;
  const m = document.getElementById("popup-asma-meaning");
  if (m) m.innerText = asma.meaning;
  const a = document.getElementById("popup-asma-ayah");
  if (a) a.innerText = asma.ayah;
  const s = document.getElementById("popup-asma-surah");
  if (s) s.innerText = asma.quran;
  const p = document.getElementById("asma-info-popup");
  if (p) p.classList.remove("hidden-popup");
}
...
function setupNameOfDay() {
  ...
  const dayOfYear = Math.floor(Date.now() / msInDay);
...
function filterSurahs() {
  ...
  renderSurahs(
    allSurahs.filter((s) => s.name.toLowerCase().includes(q) || cleanSurahName(s.name).includes(q))
  );
}
...
function renderPage() {
  ...
  p.forEach((a) => {
    ...
    sp.onclick = () => {
      triggerHaptic();
      triggerVersePopup(a.text, `سورة ${currentSurahName} - آية ${a.number}`);
    };
    txt.appendChild(sp);
  });
...
function saveReadingProgress(num, name, idx) {
  localStorage.setItem(
    "lastQuranProgress",
    JSON.stringify({ surahNum: num, surahName: cleanSurahName(name), pageIndex: idx })
  );
}
...
function copySelectedVerse() {
  triggerHaptic();
  navigator.clipboard.writeText(`﴿ ${tempSelectedVerseText} ﴾ [ ${tempSelectedVerseInfo} ]`).then(() => {
    showToast("تم النسخ");
    closeVersePopup();
  });
}
