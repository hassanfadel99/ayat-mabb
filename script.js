 ...
 * (التحديثات تعمل بالهاتف فقط + الأذان يعمل + القبلة المنعمة + الختمة)
 */ 

let APP_CURRENT_VERSION = 1.0; 
const versionMatch = window.location.href.match(/[?&]v=([0-9.]+)/);
if (versionMatch && versionMatch[1]) {
  // APP_CURRENT_VERSION = parseFloat(versionMatch[1]);
  // تم إيقاف تغيير النسخة من الرابط لضمان الفحص الصحيح للتحديثات عبر فايربيس
}
...
// ==========================================
// 🌟 نظام التحديثات (النسخة الأصلية الخاصة بك)
// ==========================================
function checkForUpdates() {
  // هذا السطر يمنع ظهور نافذة التحديث في متصفح الويب (الكمبيوتر أو كروم)
  ...
  if (dynamicUpdateUrl) {
    try {
      if (
        ...
        typeof AndroidBridge.openUpdateLink === "function"
      ) {
        AndroidBridge.openUpdateLink(dynamicUpdateUrl);
      } else {
...
const iraqCitiesCoords = {
  ...
  "Dhi Qar": { lat: 31.058, lng: 46.2573 },
  Maysan: { lat: 31.841, lng: 47.1432 },
  ...
  Sulaymaniyah: { lat: 35.55, lng: 45.4333 },
...
async function fetchPrayerTimes() {
  try {
    const coords =
      iraqCitiesCoords[currentCityKey] || iraqCitiesCoords["Basra"];
    const res = await fetch(
      `https://api.aladhan.com/v1/timings?latitude=${coords.lat}&longitude=${coords.lng}&method=0`
    );
    const data = await res.json();
    if (data.code === 200) {
      prayerTimesData = data.data.timings;
      document.getElementById("pt-fajr").innerText = format12Hour(
        prayerTimesData.Fajr
      );
      document.getElementById("pt-dhuhr").innerText = format12Hour(
        prayerTimesData.Dhuhr
      );
      document.getElementById("pt-maghrib").innerText = format12Hour(
...
function calculateNextPrayer() {
  ...
  for (let [prayerName, prayerTime] of Object.entries(prayersToAlert)) {
    if (
      adhanToggles[prayerName] &&
      currentHHMM === prayerTime &&
  ...
  const uiPrayers = [
    {
      name: "الفجر",
      timeStr: prayerTimesData.Fajr,
      time: parseTime(prayerTimesData.Fajr),
    },
    {
      name: "الظهر",
      timeStr: prayerTimesData.Dhuhr,
      time: parseTime(prayerTimesData.Dhuhr),
    },
    {
      name: "المغرب",
      timeStr: prayerTimesData.Maghrib,
      time: parseTime(prayerTimesData.Maghrib),
    },
  ];
  ...
  // 3. إرسال منبه الخلفية لنظام الأندرويد (للصلوات المفعلة فقط) بالصيغة القديمة الناجحة
  const alarmPrayers = [];
  if (adhanToggles["الفجر"])
    alarmPrayers.push({
      name: "الفجر",
      timeStr: prayerTimesData.Fajr,
      time: parseTime(prayerTimesData.Fajr),
    });
  if (adhanToggles["الظهر"])
    alarmPrayers.push({
      name: "الظهر",
      timeStr: prayerTimesData.Dhuhr,
      time: parseTime(prayerTimesData.Dhuhr),
    });
  if (adhanToggles["المغرب"])
    alarmPrayers.push({
      name: "المغرب",
      timeStr: prayerTimesData.Maghrib,
      time: parseTime(prayerTimesData.Maghrib),
  ...
  if (alarmPrayers.length > 0) {
    ...
    if (nextAlarm.name !== lastScheduledPrayer) {
      ...
      if (
        typeof AndroidBridge !== "undefined" &&
        typeof AndroidBridge.schedulePrayerAlarm === "function"
      ) {
...
function updateAdhanToggles() {
  ...
  if (
    typeof AndroidBridge !== "undefined" &&
    typeof AndroidBridge.cancelPrayerAlarm === "function"
  ) {
...
// ==========================================
// 🌟 القواميس الأساسية للتطبيق
// ==========================================

const i18n = {
  ar: {
    app_title: "تطبيق آيات",
    app_subtitle: "رحلة إيمانية في عالم القرآن الكريم",
    enter_app: "دخول التطبيق",
    menu: "القائمة",
    home: "الرئيسية",
    settings: "التخصيص والإعدادات",
    about: "حول التطبيق",
    notifications: "الإشعارات",
    ward_title: "قراءة القرآن اليومية",
    ward_calc: "جاري الاحتساب...",
    resume_title: "متابعة القراءة",
    khatma_plan: "مخطط الختمة",
    nod_title: "اسم اليوم",
    hijri_date: "التاريخ الهجري الحالي",
    quran: "تلاوة القرآن",
    quran_desc: "قراءة واستماع",
    azkar: "حصن المسلم",
    azkar_desc: "موسوعة الأذكار",
    misbaha: "المسبحة الذكية",
    misbaha_desc: "عداد الاستغفار",
    financial: "الحاسبة الشرعية",
    financial_desc: "الزكاة والخمس",
    settings_short: "التخصيص",
    settings_desc: "الواجهة واللغات",
    khatma: "الختمة",
    khatma_desc: "جدول القراءة",
    surah_list: "قائمة السور",
    choose_reader: "اختر القارئ",
    start_audio: "تشغيل التلاوة",
    copy_tip: "اضغط على الآية لنسخها",
    prev: "السابق",
    next: "التالي",
    playing: "جاري التلاوة...",
    morning: "الصباح",
    evening: "المساء",
    travel: "السفر",
    prayer: "الصلاة",
    ...
    khums: "الخمس",
    records: "السجل",
    zakat_title: "حاسبة الزكاة (2.5%)",
    total_money: "رأس المال المتوفر:",
    zakat_due: "الزكاة المستحقة:",
    save_record: "حفظ",
    khums_title: "حاسبة الخمس (20%)",
    surplus_money: "الأموال الزائدة:",
    khums_due: "الخمس الواجب:",
    imam_share: "سهم الإمام:",
    sadah_share: "سهم السادة:",
    history_title: "السجل المالي",
    khatma_duration: "أدخل المدة المستهدفة (بالأيام):",
    khatma_daily: "المطلوب يومياً:",
    khatma_start: "اعتماد",
    ward_goal_title: "قراءة القرآن اليومية",
    ward_goal_desc: "تحديد عدد الصفحات المطلوب قراءتها يومياً",
    lang_title: "لغة التطبيق",
    lang_desc: "اختر لغتك المفضلة",
    color_wheel_desc: "اسحب الشريط لتغيير لون الواجهة",
    pick_color: "اختر لون مخصص",
    template_theme: "تخصيص قالب المظهر",
    manual_theme: "تخصيص لون الواجهة",
    layout_title: "شكل وكثافة الواجهة",
    layout_desc: "5 أشكال احترافية",
    copy: "نسخ النص",
    close: "إغلاق",
    share: "مشاركة التطبيق",
    stories: "ستوريات قرآنية",
    stories_desc: "تلاوات قصيرة",
    family_khatma: "الختمة العائلية",
    family_khatma_desc: "ختمة مشتركة مع العائلة",
    qibla: "اتجاه القبلة",
    qibla_desc: "بوصلة دقيقة للكعبة",
  },
  en: {
    app_title: "Ayat App",
    app_subtitle: "A Journey in the Quran",
    enter_app: "Enter App",
    menu: "Menu",
    home: "Home",
    settings: "Settings",
    ...
    ward_title: "Daily Quran",
    ward_calc: "Calculating...",
    resume_title: "Resume Reading",
    khatma_plan: "Khatma Plan",
    nod_title: "Name of the Day",
    hijri_date: "Hijri Date",
    quran: "Holy Quran",
    quran_desc: "Read & Listen",
    azkar: "Azkar",
    azkar_desc: "Muslim Fortress",
    misbaha: "Smart Misbaha",
    misbaha_desc: "Tasbeeh Counter",
    financial: "Islamic Calculator",
    financial_desc: "Zakat & Khums",
    settings_short: "Settings",
    settings_desc: "Theme & Lang",
    khatma: "Khatma",
    khatma_desc: "Reading Schedule",
    surah_list: "Surahs List",
    choose_reader: "Select Reader",
    start_audio: "Play Audio",
    copy_tip: "Tap on verse to copy",
    prev: "Prev",
    next: "Next",
    playing: "Playing...",
    morning: "Morning",
    ...
    khums: "Khums",
    records: "Records",
    zakat_title: "Zakat Calculator (2.5%)",
    total_money: "Total Amount:",
    zakat_due: "Zakat Due:",
    save_record: "Save",
    khums_title: "Khums Calculator (20%)",
    surplus_money: "Surplus Money:",
    khums_due: "Khums Due:",
    imam_share: "Imam Share:",
    sadah_share: "Sadah Share:",
    history_title: "Financial History",
    khatma_duration: "Target Days:",
    khatma_daily: "Daily Target:",
    khatma_start: "Start",
    ward_goal_title: "Daily Quran Goal",
    ward_goal_desc: "Pages to read daily",
    lang_title: "App Language",
    template_theme: "Theme Template",
    manual_theme: "Custom Color",
    layout_title: "UI Layout",
    copy: "Copy Text",
    close: "Close",
    share: "Share App",
    stories: "Quran Stories",
    stories_desc: "Short Recitations",
    family_khatma: "Family Khatma",
    family_khatma_desc: "Shared Khatma",
    qibla: "Qibla",
    qibla_desc: "Accurate Compass",
  },
  ku: {
    app_title: "پڕۆگرامی ئايات",
    app_subtitle: "گەشتێک لە قورئاندا",
    enter_app: "چوونە ژوورەوە",
    menu: "لیست",
    home: "سەرەکی",
    settings: "ڕێکخستنەکان",
    about: "دەربارەی پڕۆگرام",
    notifications: "ئاگادارییەکان",
    ward_title: "خوێندنی ڕۆژانەی قورئان",
    ward_calc: "لەژماردندایە...",
    resume_title: "بەردەوام بوون",
    khatma_plan: "پلانى خەتمە",
    nod_title: "ناوی ڕۆژ",
    hijri_date: "بەرواری کۆچی",
    quran: "قورئانی پیرۆز",
    quran_desc: "خوێندنەوە و گوێگرتن",
    azkar: "زیکرەکان",
    azkar_desc: "قەڵای موسڵمان",
    misbaha: "تەزبیحی زیرەک",
    misbaha_desc: "ئامێری ژماردن",
    financial: "ژمێرەری ئیسلامی",
    financial_desc: "زەکات و خمس",
    settings_short: "ڕێکخستنەکان",
    settings_desc: "ڕووکار و زمان",
    khatma: "خەتمە",
    khatma_desc: "خشتەی خوێندن",
    surah_list: "لیستی سوورەتەکان",
    choose_reader: "قورئانخوێن هەڵبژێرە",
    start_audio: "لێدان",
    copy_tip: "بۆ لەبەرگرتنەوە پەنجە بنێ بە ئایەتەکەدا",
    prev: "پێشوو",
    next: "دواتر",
    playing: "دەخوێندرێتەوە...",
    morning: "بەیانیان",
    evening: "ئێواران",
    travel: "سەفەر",
    prayer: "نوێژ",
    tap_to_count: "پەنجە بنێ بۆ ژماردن",
    reset: "سفرکردنەوە",
    zakat: "زەکات",
    khums: "خمس",
    records: "تۆمارەکان",
    zakat_title: "حاسبەی زەکات (2.5%)",
    total_money: "بڕی پارە:",
    zakat_due: "زەکاتی پێویست:",
    save_record: "پاشەکەوت",
    khums_title: "حاسبەی خمس (20%)",
    surplus_money: "پارەی زیادە:",
    khums_due: "خمس پێویست:",
    imam_share: "بەشی ئیمام:",
    sadah_share: "بەشی سادە:",
    history_title: "تۆماری دارایی",
    khatma_duration: "ژمارەی ڕۆژەکان:",
    khatma_daily: "پێویستی ڕۆژانە:",
    khatma_start: "دەستپێکردن",
    ward_goal_title: "ئامانجی ڕۆژانەی قورئان",
    ward_goal_desc: "ژمارەی لاپەڕەکان دیاری بکە",
    lang_title: "زمانی پڕۆگرام",
    template_theme: "ڕووکار",
    manual_theme: "ڕەنگی دڵخواز",
    layout_title: "شێوازی ڕووکار",
    copy: "لەبەرگرتنەوە",
    close: "داخستن",
    share: "بڵاوکردنەوە",
    stories: "ستۆری قورئانی",
    stories_desc: "خوێندنەوەی کورت",
    family_khatma: "خەتمەی خێزانی",
    family_khatma_desc: "خەتمەی هاوبەش",
    qibla: "ڕووگە",
    qibla_desc: "بوصلەی ڕووگە",
  },
  fa: {
    app_title: "برنامه آیات",
    app_subtitle: "سفری در قرآن کریم",
    ...
    ward_title: "تلاوت روزانه قرآن",
    ward_calc: "در حال محاسبه...",
    resume_title: "ادامه تلاوت",
    khatma_plan: "برنامه ختم",
    nod_title: "نام امروز",
    hijri_date: "تاریخ هجری",
    quran: "قرآن کریم",
    quran_desc: "خواندن و شنیدن",
    azkar: "اذکار",
    azkar_desc: "حصن المسلم",
    misbaha: "تسبیح هوشمند",
    misbaha_desc: "شمارشگر",
    financial: "ماشین حساب اسلامی",
    financial_desc: "زکات و خمس",
    settings_short: "تنظیمات",
    settings_desc: "پوسته و زبان",
    khatma: "ختم قرآن",
    khatma_desc: "برنامه زمان‌بندی",
    surah_list: "فهرست سوره‌ها",
    choose_reader: "انتخاب قاری",
    start_audio: "پخش",
    copy_tip: "برای کپی روی آیه کلیک کنید",
    prev: "قبلی",
    next: "بعدی",
    playing: "در حال پخش...",
    morning: "صبح",
    ...
    travel: "سفر",
    prayer: "نماز",
    tap_to_count: "برای شمارش کلیک کنید",
    reset: "بازنشانی",
    zakat: "زکات",
    khums: "خمس",
    records: "سوابق",
    zakat_title: "محاسبه زکات (2.5%)",
    total_money: "مبلغ کل:",
    zakat_due: "زکات واجب:",
    save_record: "ذخیره",
    khums_title: "محاسبه خمس (20%)",
    surplus_money: "اموال مازاد:",
    khums_due: "خمس واجب:",
    imam_share: "سهم امام:",
    sadah_share: "سهم سادات:",
    history_title: "سوابق مالی",
    khatma_duration: "تعداد روزها:",
    khatma_daily: "هدف روزانه:",
    khatma_start: "تایید",
    ward_goal_title: "هدف روزانه قرآن",
    ward_goal_desc: "تعداد صفحات تلاوت روزانه",
    lang_title: "زبان برنامه",
    template_theme: "پوسته",
    manual_theme: "رنگ دلخواه",
    layout_title: "چیدمان رابط کاربری",
    copy: "کپی متن",
    close: "بستن",
    share: "اشتراک گذاری",
    stories: "استوری‌های قرآنی",
    stories_desc: "تلاوت‌های کوتاه",
    family_khatma: "ختم خانواده",
    family_khatma_desc: "ختم مشترک",
    qibla: "قبله نما",
    qibla_desc: "قطب نمای دقیق",
  },
};
...
function changeLanguage(lang) {
  ...
  document
    .querySelectorAll(".lang-group button")
...
function updateSeekbarSmoothly() {
  if (quranAudioEl && !quranAudioEl.paused) {
    if (seekbar && !seekbar.isDragging) {
      ...
      if (currentTimeEl)
        currentTimeEl.innerText = formatTime(quranAudioEl.currentTime);
    }
...
if (quranAudioEl) {
  ...
  if (seekbar) {
    ...
    const endDrag = () => {
      ...
      quranAudioEl.currentTime = seekbar.value;
    };
  ...
  quranAudioEl.onplay = () => {
    ...
    const btnPlay = document.getElementById("f-btn-play");
    if (btnPlay) btnPlay.innerHTML = '<i class="fa-solid fa-pause"></i>';
    const icon = document.getElementById("f-audio-icon");
  ...
  if (id !== "stories-section") {
    document.querySelectorAll(".story-video").forEach((vid) => {
      ...
      const overlay = document.getElementById(
        vid.id.replace("story-vid", "story-overlay")
      );
      if (overlay) overlay.classList.remove("playing");
      const miniBtn = document.getElementById(
        vid.id.replace("story-vid-", "vid-play-btn-")
      );
...
async function loadSurahs() {
  try {
    ...
    if (c)
      c.innerHTML =
        "<div style='grid-column: 1/-1; text-align:center; padding:20px;'>تعذر التحميل، يرجى التحقق من الإنترنت.</div>";
  }
}
...
function renderSurahs(list) {
  ...
  list.forEach((s) => {
    ...
    btn.innerHTML = `<div class="surah-number">${
      s.number
    }</div><div class="surah-name">سورة ${cleanSurahName(s.name)}</div>`;
    btn.onclick = () => {
      triggerHaptic();
      selectSurah(s.number, s.name);
    };
...
function filterSurahs() {
  ...
  renderSurahs(
    allSurahs.filter(
      (s) =>
        s.name.toLowerCase().includes(q) || cleanSurahName(s.name).includes(q)
    )
  );
}
...
function renderPage() {
  ...
  p.forEach((a) => {
    ...
    sp.onclick = () => {
      ...
      triggerVersePopup(
        a.text,
        `سورة ${currentSurahName} - آية ${a.number}`
      );
    };
    txt.appendChild(sp);
  });
...
function changeFontSize(amt) {
  ...
  document
    .querySelectorAll(".ayah-box")
    .forEach((s) => (s.style.fontSize = defaultFontSize + "px"));
}
...
function updateWardDashboardProgress() {
  ...
  try {
    ...
    if (stored) {
      ...
      if (Array.isArray(parsed)) rd = parsed;
    }
  } catch (e) {
  ...
  if (svd && resBox) {
    ...
    if (resTxt)
      resTxt.innerHTML = `سورة ${cleanSurahName(pr.surahName)} - صفحة ${
        pr.pageIndex + 1
      }`;
...
function switchAzkarCategory(cat) {
  ...
  document
    .querySelectorAll(".azkar-category-tabs-scroll .tab-btn")
    .forEach((b) => b.classList.remove("active"));
  const t = document.getElementById(`tab-${cat}`);
  if (t) t.classList.add("active");
  renderAzkarList(fullAzkarDatabase[cat]);
}

function renderAzkarList(list) {
  const c = document.getElementById("azkar-container");
  if (!c || !list) return;
  c.innerHTML = "";
  list.forEach((z, i) => {
    const d = document.createElement("div");
    d.className = "zekr-card";
    d.innerHTML = `<div class="zekr-text">${z.text}</div><button id="counter-${currentAzkarCategory}-${i}" class="zekr-counter-btn haptic-btn">المتبقي: ${z.count}</button>`;
    const b = d.querySelector("button");
    if (b) b.onclick = () => countZekr(currentAzkarCategory, i);
    c.appendChild(d);
  });
}

function filterAzkar() {
  const inp = document.getElementById("azkar-search-input");
  if (!inp) return;
  const q = inp.value.trim().toLowerCase();
  if (!q) renderAzkarList(fullAzkarDatabase[currentAzkarCategory]);
  else
    renderAzkarList(
      fullAzkarDatabase[currentAzkarCategory].filter((z) =>
        z.text.toLowerCase().includes(q)
      )
    );
}
...
function countZekr(cat, i) {
  triggerHaptic();
  const b = document.getElementById(`counter-${cat}-${i}`);
  if (!b) return;
  let n = parseInt(b.innerText.replace(/[^\d]/g, ""));
  if (n > 1) {
    n--;
    b.innerText = `المتبقي: ${n}`;
  } else {
    b.innerText = "تمت القراءة ✓";
    b.className = "zekr-counter-btn done";
    b.disabled = true;
  }
}
...
function activateKhatmaChallenge() {
  ...
  if (isNaN(d) || d <= 0) {
    showToast("الرجاء إدخال مدة صحيحة");
    return;
  }
  localStorage.setItem(
    "active_khatma_challenge",
    JSON.stringify({ days: d, pagesPerDay: Math.ceil(604 / d) })
  );
...
function switchFinancialTab(t) {
  triggerHaptic();
  document
    .querySelectorAll(".f-capsule-btn")
    .forEach((b) => b.classList.remove("active"));
  document
    .querySelectorAll(".sub-financial-panel")
...
function saveToFinancialHistory(type) {
  ...
  if (type === "زكاة") {
    ...
    const zOut = document.getElementById("zakat-output");
    amt = zInp ? parseFloat(zInp.value) : 0;
    right = zOut ? parseFloat(zOut.innerText) : 0;
  } else {
    const kInp = document.getElementById("khums-amount-input");
    const kOut = document.getElementById("khums-total-output");
    amt = kInp ? parseFloat(kInp.value) : 0;
    right = kOut ? parseFloat(kOut.innerText) : 0;
  }
  if (isNaN(amt) || amt <= 0) {
    showToast("أدخل مبلغ صحيح");
    return;
  }
  const h =
    JSON.parse(localStorage.getItem("financial_history_records")) || [];
...
function renderFinancialHistory() {
  ...
  h.forEach((i, index) => {
    ...
    c.innerHTML += `<div class="history-item-row" style="display: flex; justify-content: space-between; align-items: center; background: var(--bg); padding: 12px 15px; border-radius: 12px; margin-bottom: 10px; border: 1px solid var(--border);"><div class="history-meta-info"><h5 style="margin: 0 0 5px 0; font-size: 1rem; color: var(--primary);">${
      i.type
    }</h5><span style="font-size: 0.8rem; opacity: 0.8; color: var(--text);">${
      i.date
    }</span></div><div style="display: flex; align-items: center; gap: 15px;"><div class="history-val" style="font-weight: bold; color: var(--text); font-size: 1.1rem;">★ ${safeRight.toFixed(
      2
    )}</div><button onclick="deleteFinancialRecord(${index})" class="haptic-btn" style="background: transparent; border: none; color: #e63946; font-size: 1.2rem; cursor: pointer; padding: 5px;"><i class="fa-solid fa-trash-can"></i></button></div></div>`;
  });
...
function clearFinancialHistory() {
  ...
  const msg = document.getElementById("delete-popup-msg");
  if (msg)
    msg.innerText = "هل أنت متأكد من مسح جميع السجلات؟ لا يمكن التراجع عن هذه الخطوة.";
  const popup = document.getElementById("delete-confirm-popup");
  if (popup) popup.classList.remove("hidden-popup");
}
...
function triggerVersePopup(text, info) {
  tempSelectedVerseText = text;
  tempSelectedVerseInfo = info;
  const pTxt = document.getElementById("popup-verse-text");
  if (pTxt) pTxt.innerText = `﴿ ${text} ﴾`;
  const pInfo = document.getElementById("popup-verse-info");
  if (pInfo) pInfo.innerText = info;
  const vPopup = document.getElementById("verse-action-popup");
  if (vPopup) vPopup.classList.remove("hidden-popup");
}

function closeVersePopup() {
  triggerHaptic();
  const vPopup = document.getElementById("verse-action-popup");
  if (vPopup) vPopup.classList.add("hidden-popup");
}

function copySelectedVerse() {
  triggerHaptic();
  navigator.clipboard
    .writeText(`﴿ ${tempSelectedVerseText} ﴾ [ ${tempSelectedVerseInfo} ]`)
    .then(() => {
      showToast("تم النسخ");
      closeVersePopup();
    });
...
function setupNotificationsListener() {
  ...
  database.ref("notifications").on("value", (snapshot) => {
    ...
    const data = snapshot.val();
    if (!data) {
      container.innerHTML = `<div style="text-align: center; padding: 20px; opacity: 0.6; font-size: 0.9rem; color: var(--text);">لا توجد إشعارات حالياً</div>`;
      return;
    }
    const messages = [];
    for (let key in data) {
    ...
    messages.reverse().forEach((msg) => {
      container.innerHTML += `<div style="background: var(--bg); border: 1px solid var(--border); padding: 12px 15px; border-radius: 14px; margin-bottom: 10px;"><div style="font-weight: bold; color: var(--primary); font-size: 0.85rem; margin-bottom: 5px; display: flex; justify-content: space-between;"><span>إدارة التطبيق</span><span style="opacity: 0.7; font-size: 0.75rem;">${
        msg.time || ""
      }</span></div><p style="margin: 0; font-size: 0.95rem; color: var(--text); line-height: 1.5;">${
        msg.text
      }</p></div>`;
    });
  });
...
function setupStoriesListener() {
  ...
  database.ref("stories").on("value", (snapshot) => {
    ...
    if (data) {
      for (let key in data) {
        allStoriesData.push({ id: key, url: data[key].url });
      }
    }
  });
}
    ...
    document.querySelectorAll(".story-video").forEach((vid) => {
      if (vid !== video) {
        ...
        const ov = document.getElementById(
          vid.id.replace("vid", "overlay")
        );
        if (ov) ov.classList.remove("playing");
        const mb = document.getElementById(
          vid.id.replace("story-vid-", "vid-play-btn-")
        );
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          ...
          const miniPlayBtn = document.getElementById(
            vid.id.replace("story-vid-", "vid-play-btn-")
          );
            ...
            vid
              ...
              .then(() => {
                ...
                const nextVid = document.getElementById(
                  `story-vid-${currentIdNum + 1}`
                );
                if (
                  nextVid &&
                  !nextVid.getAttribute("src") &&
                  nextVid.getAttribute("data-src")
                ) {
                  nextVid.setAttribute(
                    "src",
                    nextVid.getAttribute("data-src")
                  );
...
window.shareApp = function () {
  triggerHaptic();
  const shareText =
    "تطبيق آيات: رحلة إيمانية متكاملة في القرآن الكريم، الأذكار، والستوريات.\nhttps://hassanfadel99.github.io/maayat/";
  if (
    typeof window.AndroidBridge !== "undefined" &&
    typeof window.AndroidBridge.shareApp === "function"
  ) {
    ...
    navigator.clipboard
      .writeText(shareText)
      .then(() => showToast("تم نسخ الرابط للحافظة"));
  }
};
...
window.directDownloadVideo = async function (event, url) {
  ...
  if (
    typeof window.AndroidBridge !== "undefined" &&
    typeof window.AndroidBridge.downloadVideo === "function"
  ) {
  ...
  try {
    ...
    a.href = blobUrl;
    a.download = "ستوريات_تطبيق_آيات_" + new Date().getTime() + ".mp4";
...
window.cancelDownload = function () {
  ...
  if (
    typeof window.AndroidBridge !== "undefined" &&
    typeof window.AndroidBridge.cancelDownloadVideo === "function"
  ) {
...
window.updateDownloadProgress = function (percent) {
  ...
  const textEl = document.getElementById("download-text");
  const barEl = document.getElementById("download-bar-fill");
  if (overlay && overlay.classList.contains("hidden-popup"))
...
// ==========================================
// 🌟 دوال نافذة الـ GPS والقبلة الدقيقة + نافذة المعايرة (رقم 8)
// ==========================================
...
function openGPSSetting() {
    ...
    try {
        if (typeof window.AndroidBridge !== "undefined" && typeof window.AndroidBridge.openLocationSettings === "function") {
            window.AndroidBridge.openLocationSettings();
        }
    } catch (e) { console.log(e); }
    
    isWaitingForGPS = true;
    document.getElementById('qibla-status').innerText = "بانتظار تشغيل الموقع... للعودة اضغط رجوع";
}
...
document.addEventListener("visibilitychange", function() {
    if (document.visibilityState === 'visible' && isWaitingForGPS) {
        isWaitingForGPS = false;
        document.getElementById('gps-prompt-popup').classList.add('hidden-popup');
        initQibla(); 
    }
...
let currentDisplayedHeading = 0; 
let hasShownCalibration = false;

// 🌟 نافذة المعايرة (تنبيه المستخدم لرسم رقم 8)
function showCalibrationPopup() {
    if (hasShownCalibration) return;
    hasShownCalibration = true;

    let calibPopup = document.createElement("div");
    calibPopup.id = "calibration-popup";
    calibPopup.className = "verse-popup-overlay";
    calibPopup.innerHTML = `
        <div class="perfect-square-popup" style="text-align: center; gap: 10px; padding: 25px 20px;">
            <div style="font-size: 4.5rem; color: var(--accent); margin-bottom: 15px; animation: rotate8 2.5s infinite ease-in-out;">
                <i class="fa-solid fa-infinity"></i>
            </div>
            <h3 style="color: var(--text); font-family: 'Cairo'; font-size: 1.4rem; margin-bottom: 10px;">معايرة البوصلة</h3>
            <p style="font-size: 0.95rem; opacity: 0.85; color: var(--text); margin-bottom: 25px; line-height: 1.6;">
                للحصول على دقة عالية لاتجاه القبلة، يرجى تحريك هاتفك في الهواء على شكل <strong>رقم 8 بالإنجليزية</strong> مرتين أو ثلاث مرات، وتأكد من الابتعاد عن أي معادن.
            </p>
            <div class="verse-popup-buttons">
                <button class="v-btn haptic-btn" style="background: var(--primary); color: white;" onclick="closeCalibrationPopup()">حسناً، فهمت</button>
            </div>
        </div>
        <style>
            @keyframes rotate8 {
                0% { transform: rotate(0deg) scale(1); }
                50% { transform: rotate(180deg) scale(1.15); color: #2ecc71; }
                100% { transform: rotate(360deg) scale(1); }
            }
        </style>
    `;
    document.body.appendChild(calibPopup);
}

window.closeCalibrationPopup = function() {
    triggerHaptic();
    const calibPopup = document.getElementById("calibration-popup");
    if (calibPopup) calibPopup.remove();
};
...
function initQibla() {
    ...
    statusEl.innerText = "جاري طلب الموقع وتحديد القبلة...";
    statusEl.style.color = "var(--text)";
    
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            (position) => {
                // إحداثيات موقع المستخدم الدقيق
                ...
                startCompassSensor();
                showCalibrationPopup(); // إظهار التنبيه بتحريك الهاتف
            },
            (error) => {
...
// 🌟 خوارزمية حساب القبلة الهندسية (المسافة القوسية الصغرى)
...
function startCompassSensor() {
    ...
    if (window.DeviceOrientationEvent) {
        ...
        document.getElementById('qibla-status').innerText = "هاتفك لا يحتوي على حساس بوصلة (Magnetometer).";
        document.getElementById('qibla-status').style.color = "#e63946";
    }
}
...
function handleOrientation(event) {
    ...
    if (event.webkitCompassHeading) {
        ...
        isValid = true;
    } else if (event.absolute && alpha !== null) {
    ...
    if (disc) {
        let targetHeading = qiblaBearing - compassHeading;
        let diff = ((targetHeading - currentDisplayedHeading + 180) % 360) - 180; 
        
        // 🌟 تنعيم حركة البوصلة (Low Pass Filter) لمنع الارتداد السريع
        currentDisplayedHeading += diff * 0.15; 

        disc.style.transform = `rotate(${currentDisplayedHeading}deg)`;
...
function checkCurrentRoom() {
    ...
    fkRoomListener = database.ref('family_khatmas/' + currentFkRoomId).on('value', snapshot => {
        ...
        try {
            if (typeof window.AndroidBridge !== "undefined" && typeof window.AndroidBridge.sendLocalNotification === "function") {
                if (!isFkInitialLoad) {
