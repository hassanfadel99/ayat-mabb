 ...
 * النسخة المحدثة (3.2) - نظام الختمة العائلية بأزرار وتصميمات احترافية
 */ 

let APP_CURRENT_VERSION = 3.2; 
...
const i18n = { 
  ar: { 
    ...
    family_khatma: "الختمة العائلية", family_khatma_desc: "ختمة مشتركة مع العائلة",
...
function renderPortionsGrid(room) {
    ...
    const prefix = room.type === 'ajza' ? 'الجزء' : 'سورة';
    const totalItems = room.type === 'ajza' ? 30 : 114;
    ...
    for (let i = 1; i <= totalItems; i++) {
        ...
        if (userStats[portion.assignedToUid]) {
            ...
            userStats[portion.assignedToUid].portions.push({ id: i, status: portion.status });
        }
    ...
    Object.values(userStats).forEach(stat => {
        ...
        stat.portions.forEach(portion => {
            ...
            let icon = portion.status === 'completed' ? '<i class="fa-solid fa-check"></i> ' : '';
            chipsHtml += `<div class="${chipClass}">${icon}${portion.id}</div>`;
        });
