...
// ==========================================
// 🌟 7. البث المباشر (الديناميكي - تم التحديث للمربعات) 🌟
// ==========================================
window.playDynamicLiveChannel = function(channelKey, defaultUrl, btnElement) { 
    triggerHaptic(); 
    
    // إزالة كلاس active من أي أزرار أو مربعات سابقة
    document.querySelectorAll('.tv-btn, .tv-channel-card').forEach(btn => btn.classList.remove('active')); 
...
window.downloadAudioFile = function(event, url, title) { 
    ...
    document.getElementById("download-progress-overlay").classList.remove("hidden-popup"); 
    document.getElementById("download-text").innerText = "بدأ التحميل... 0%"; 
    document.getElementById("download-bar-fill").style.width = "0%"; 

    let fullUrl = url; 
    if (!url.startsWith("http")) fullUrl = window.location.origin + window.location.pathname.replace("index.html", "") + url; 
...
window.directDownloadVideo = async function (event, url) { 
    ...
    try { 
        ...
        else { if (overlay) overlay.classList.add("hidden-popup"); window.open(url, "_blank"); } 
    } 
}; 
