/** 
 * تطبيق آيات - الملف البرمجي الشامل (النسخة النهائية مع الإحصاءات والبروفايل)
 */ 
...
let totalMisbahaCounter = parseInt(localStorage.getItem("ayat_total_misbaha")) || 0; // المجموع الكلي
...
// ==========================================
// 7. البث المباشر (الديناميكي - متوافق مع المربعات)
// ==========================================
...
// 🌟 تحديث المسبحة لتعمل مع المجموع الكلي 🌟
window.incrementMisbaha = function() { 
    misbahaCounter++; 
    totalMisbahaCounter++; 
    
    // حفظ المجموع الكلي
    localStorage.setItem("ayat_total_misbaha", totalMisbahaCounter);
    
    const num = document.getElementById("misbaha-count-number"); 
    if (num) num.innerText = misbahaCounter; 
    
    // تحديث الواجهة مباشرة إذا كان فاتح الملف الشخصي
    const profileNum = document.getElementById("profile-zekr-count");
    if (profileNum) profileNum.innerText = totalMisbahaCounter;

    if (navigator.vibrate) triggerHaptic(); 
} 

window.resetMisbaha = function() { 
    triggerHaptic(); 
    misbahaCounter = 0; // يتم تصفير العداد الحالي فقط
    const num = document.getElementById("misbaha-count-number"); 
    if (num) num.innerText = misbahaCounter; 
} 
...
// ==========================================
// 14. 🌟 نظام المصادقة (Authentication) والملف الشخصي 🌟
// ==========================================
...
window.registerUser = function() {
    triggerHaptic();
    const name = document.getElementById("auth-name-signup").value.trim();
    const email = document.getElementById("auth-email-signup").value.trim();
    const password = document.getElementById("auth-pass-signup").value.trim();
    ...
    if (typeof firebase === "undefined" || !firebase.auth) {
        showToast("خدمة المصادقة غير متوفرة حالياً");
        return;
    }
...
window.loginUser = function() {
    ...
    const password = document.getElementById("auth-pass-login").value.trim();
    ...
    firebase.auth().signInWithEmailAndPassword(email, password)
        ...
        .catch((error) => {
            showToast("فشل الدخول: تأكد من الإيميل وكلمة المرور");
        });
};
...
// فحص حالة المستخدم وتحديث إحصائيات البروفايل
function checkAuthState() {
    ...
    firebase.auth().onAuthStateChanged((user) => {
        ...
        if (user) {
            ...
            if (nameEl) nameEl.innerText = user.displayName || "مستخدم آيات";
            if (emailEl) emailEl.innerText = user.email || "";

            // تحميل الصورة (الأولوية للمحفوظة محلياً لسرعة الظهور)
            const localAvatar = localStorage.getItem("user_avatar_" + user.uid);
            if (localAvatar && avatarImg) {
                avatarImg.src = localAvatar;
            } else if (user.photoURL && avatarImg) {
                avatarImg.src = user.photoURL;
            } else if (avatarImg) {
                avatarImg.src = "https://i.ibb.co/yFY29WhK/image.jpg";
            }
            
            // تحديث الإحصاءات في البروفايل
            const khatmaCount = JSON.parse(localStorage.getItem("fk_history") || "[]").length;
            const zekrCountEl = document.getElementById("profile-zekr-count");
            const khatmaCountEl = document.getElementById("profile-khatma-count");
            
            if(khatmaCountEl) khatmaCountEl.innerText = khatmaCount;
            if(zekrCountEl) zekrCountEl.innerText = totalMisbahaCounter;
...
// 🌟 برمجة زر الكاميرا (الاستوديو) وضغط الصورة 🌟
window.addEventListener("DOMContentLoaded", () => { 
    ...
    try { checkAuthState(); } catch (e) {} 

    // التحكم في رفع الصورة الشخصية وضغطها
    try {
        ...
        if (avatarInput) {
            avatarInput.addEventListener("change", function(e) {
                ...
                reader.onload = function(event) {
                    const img = new Image();
                    img.onload = function() {
                        // ضغط الصورة باستخدام Canvas لعدم استهلاك مساحة
                        ...
                        // استخراج الصورة المضغوطة بصيغة JPEG
                        const dataUrl = canvas.toDataURL("image/jpeg", 0.7);

                        // 1. حفظها محلياً في الجهاز لسرعة الفتح
                        localStorage.setItem("user_avatar_" + user.uid, dataUrl);
                        
                        // 2. تحديث الواجهة مباشرة
                        const avatarImg = document.getElementById("profile-avatar-img");
                        if (avatarImg) avatarImg.src = dataUrl;

                        // 3. تحديثها في الفايربيس للعمل على أجهزة أخرى
                        user.updateProfile({ photoURL: dataUrl }).catch(()=>{});
                        if (database) {
