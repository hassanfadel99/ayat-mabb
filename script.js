/** 
 * تطبيق آيات - الملف البرمجي الشامل (النسخة المستقرة - مع التخزين المخبأ للصلوات)
 */ 
...
let totalMisbahaCounter = parseInt(localStorage.getItem("ayat_total_misbaha")) || 0; 
...
// 🌟 إعدادات التاريخ الهجري (تم جعل الزيادة +1 يوماً هي الافتراضية) 🌟
let savedHijriAdj = localStorage.getItem('ayat_hijri_adj');
let hijriAdjustment = savedHijriAdj !== null ? parseInt(savedHijriAdj) : 1;
...
window.showSection = function (id) { 
    ...
    if (id === "calendar-section") initHijriCalendar(); 
...
// ==========================================
// 6. نظام مواقيت الصلاة والأذان 🌟 (مع نظام الكاش المطور) 🌟
// ==========================================
...
async function fetchPrayerTimes() { 
    ...
    // 1. محاولة جلب التواقيت من الكاش (الذاكرة المؤقتة) لسرعة العرض وتفادي انقطاع النت
    let cachedData = localStorage.getItem("ayat_cached_prayers_" + currentCityKey);
    if (cachedData) {
        try {
            let parsedCache = JSON.parse(cachedData);
            if (parsedCache.date === todayStr && parsedCache.timings) {
                prayerTimesData = parsedCache.timings;
                updatePrayerUI(prayerTimesData);
                startPrayerCountdown(); // تشغيل المؤقت فورا من الكاش
            }
        } catch(e) {}
    }

    // 2. جلب التواقيت من الإنترنت للتأكيد والتحديث
    try { 
        ...
        if (data.code === 200) { 
            ...
            // حفظ التواقيت في الذاكرة لتجنب التعليق مستقبلاً
            localStorage.setItem("ayat_cached_prayers_" + currentCityKey, JSON.stringify({
                date: todayStr,
            ...
            throw new Error("API Error"); // إجبار الكود للذهاب للخطأ إذا فشل الخادم
        }
    } catch (e) {
        // في حالة فشل الخادم أو الإنترنت، ولم يكن لدينا كاش مسبق
        if (!prayerTimesData) {
            const elName = document.getElementById("next-prayer-name"); 
            const elCount = document.getElementById("prayer-countdown"); 
            if (elName) elName.innerText = "تعذر جلب التواقيت";
            if (elCount) elCount.innerText = "يرجى التحقق من الشبكة";
        }
    } 
} 
...
function updatePrayerUI(timings) {
    ...
    document.getElementById("pt-fajr").innerText = format12Hour(cleanTime(timings.Fajr)); 
    document.getElementById("pt-dhuhr").innerText = format12Hour(cleanTime(timings.Dhuhr)); 
    document.getElementById("pt-maghrib").innerText = format12Hour(cleanTime(timings.Maghrib)); 
}
...
// استخدام setInterval لضمان عدم توقف المؤقت في الـ WebView
...
function calculateNextPrayer() { 
    ...
    // تشغيل الأذان بمجرد دخول الدقيقة مباشرة (لا مجال للتأخير)
...
function initPrayerSettings() { 
    ...
    // ضبط الخيار في صفحة الإعدادات
    const adjSelect = document.getElementById("hijri-adjustment-select");
    if(adjSelect) adjSelect.value = hijriAdjustment.toString();
} 
...
// ==========================================
// 🌟 6.5 التقويم الهجري الشامل والاحترافي 🌟
// ==========================================
const hijriMonthsNames = ["محرم", "صفر", "ربيع الأول", "ربيع الآخر", "جمادى الأولى", "جمادى الآخرة", "رجب", "شعبان", "رمضان", "شوال", "ذو القعدة", "ذو الحجة"];

let currentViewedHijriYear = 1445;
let currentViewedHijriMonth = 0;

function toArabicNumerals(numStr) {
    const arabicNumbers = ['٠','١','٢','٣','٤','٥','٦','٧','٨','٩'];
    return numStr.toString().replace(/[0-9]/g, w => arabicNumbers[w]);
}

window.updateHijriAdjustment = function(val) {
    triggerHaptic();
    hijriAdjustment = parseInt(val) || 1; // الافتراضي +1
    localStorage.setItem("ayat_hijri_adj", hijriAdjustment);
    showToast("تم تحديث التقويم بنجاح");
    calculateHijriDate(); 
    
    // تحديث نافذة التقويم إذا كانت مفتوحة
    if (document.getElementById("calendar-section") && !document.getElementById("calendar-section").classList.contains("hidden")) {
        initHijriCalendar();  
    }
}

function getHijriMonthInfo(year, monthIndex) {
    let isLeap = (year * 11 + 14) % 30 < 11;
    let daysInMonth = (monthIndex % 2 === 0) ? 30 : 29;
    if (monthIndex === 11 && isLeap) daysInMonth = 30;

    let daysSinceEpoch = Math.floor((year - 1) * 354.36667);
    for (let i = 0; i < monthIndex; i++) {
        daysSinceEpoch += (i % 2 === 0) ? 30 : 29;
    }
    
    let firstDayOfWeek = (5 + daysSinceEpoch) % 7; 
    return { days: daysInMonth, startDay: firstDayOfWeek };
}

window.navigateHijriMonth = function(direction) {
    triggerHaptic();
    currentViewedHijriMonth += direction;
    
    if (currentViewedHijriMonth > 11) {
        currentViewedHijriMonth = 0;
        currentViewedHijriYear++;
    } else if (currentViewedHijriMonth < 0) {
        currentViewedHijriMonth = 11;
        currentViewedHijriYear--;
    }
    updateCalendarView();
}

window.initHijriCalendar = function() {
    let d = new Date();
    d.setDate(d.getDate() + hijriAdjustment); 
    
    let formatter = new Intl.DateTimeFormat("en-US-u-ca-islamic-civil", { year: "numeric", month: "numeric", day: "numeric" }).format(d);
    let parts = formatter.split("/"); 
    currentViewedHijriMonth = parseInt(parts[0]) - 1; 
    currentViewedHijriYear = parseInt(parts[2].split(" ")[0]); 
...
function updateCalendarView() {
    ...
    const heroHijri = document.getElementById("calendar-hero-hijri");
    if(heroHijri) {
        let hijriFormatter = new Intl.DateTimeFormat("ar-SA", { calendar: "islamic-civil", day: "numeric", month: "long", year: "numeric" }).format(adjustedDate);
        heroHijri.innerText = hijriFormatter;
    }
    
    const heroGreg = document.getElementById("calendar-hero-gregorian");
    if(heroGreg) {
        let gregFormatter = new Intl.DateTimeFormat("ar-IQ", { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }).format(realDate);
        heroGreg.innerText = gregFormatter;
    }

    const monthLabel = document.getElementById("calendar-current-month-label");
    if(monthLabel) monthLabel.innerText = `${hijriMonthsNames[currentViewedHijriMonth]} ${toArabicNumerals(currentViewedHijriYear)}`;

    const info = getHijriMonthInfo(currentViewedHijriYear, currentViewedHijriMonth);
    ...
    for (let i = 0; i < info.startDay; i++) {
        ...
        emptyDiv.className = "calendar-day-box empty-day";
        grid.appendChild(emptyDiv);
    }

    let formatterEn = new Intl.DateTimeFormat("en-US-u-ca-islamic-civil", { year: "numeric", month: "numeric", day: "numeric" }).format(adjustedDate);
    let partsEn = formatterEn.split("/"); 
    let realDay = parseInt(partsEn[1]);
    let realMonth = parseInt(partsEn[0]) - 1;
    let realYear = parseInt(partsEn[2].split(" ")[0]);

    for (let day = 1; day <= info.days; day++) {
        let dayDiv = document.createElement("div");
        dayDiv.className = "calendar-day-box";
        dayDiv.innerText = toArabicNumerals(day);
        
        if (day === realDay && currentViewedHijriMonth === realMonth && currentViewedHijriYear === realYear) {
...
// ==========================================
// 7. البث المباشر المربع
// ==========================================
window.playDynamicLiveChannel = function(channelKey, defaultUrl, btnElement) { 
    triggerHaptic(); 
    document.querySelectorAll('.tv-channel-card').forEach(btn => btn.classList.remove('active')); 
...
// ==========================================
// 9. مكتبة الأدعية والمشغل الكامل
// ==========================================
...
window.downloadAudioFile = async function(event, url, title) { 
    ...
    if (typeof window.AndroidBridge !== "undefined" && typeof window.AndroidBridge.downloadAudio === "function") { 
        showToast("بدأ تحميل الدعاء في الخلفية...");
    ...
    try {
        ...
        if (!total && window.updateDownloadProgress) window.updateDownloadProgress(100);
    } catch (error) {
...
window.directDownloadVideo = async function (event, url) { 
    ...
    if (typeof window.AndroidBridge !== "undefined" && typeof window.AndroidBridge.downloadVideo === "function") { 
        showToast("بدأ التحميل.. سيتم حفظ المقطع في الاستوديو");
        window.AndroidBridge.downloadVideo(url); 
        return; 
    } 
...
window.cancelDownload = function () { 
    triggerHaptic(); 
    if (typeof window.AndroidBridge !== "undefined" && typeof window.AndroidBridge.cancelDownloadVideo === "function") { 
        window.AndroidBridge.cancelDownloadVideo(); 
...
// ==========================================
// 13. الختمة العائلية (الفايربيس) مع تصليح التجميد
// ==========================================
let fkMyUid = localStorage.getItem("fk_my_uid") || ("user_" + Math.random().toString(36).substr(2, 9));
...
window.promptLeaveFkRoom = function() { 
    triggerHaptic(); 
...
window.shareRoomCode = function() { 
    ...
    if (currentFkRoomId) { 
        ...
        if (typeof window.AndroidBridge !== "undefined" && typeof window.AndroidBridge.shareApp === "function") { 
            ...
            navigator.clipboard.writeText(currentFkRoomId).then(() => { showToast("تم نسخ كود الغرفة للمشاركة"); }); 
        } 
    } 
} 

// 🌟 حل مشكلة التجميد في زر التوزيع 🌟
window.startFamilyDistribution = function() { 
    triggerHaptic(); 
    if (!database || !currentFkRoomId) return; 
    
    const distBtn = document.getElementById("fk-start-distribution-btn");
    if (distBtn) {
        distBtn.innerHTML = `<i class="fa-solid fa-spinner fa-spin"></i> جاري التوزيع ⏳`;
    ...
    database.ref("family_khatmas/" + currentFkRoomId).once("value").then((snapshot) => { 
        ...
        if (!room || !room.participants) {
            if (distBtn) { distBtn.innerHTML = `<i class="fa-solid fa-shuffle"></i> توزيع الأجزاء وبدء الختمة`; distBtn.disabled = false; }
            return showToast("لا يوجد مشاركون!");
        }
        ...
        for (let i = 1; i <= totalItems; i++) { 
            ...
            portions[i] = { id: i, assignedToUid: uid, assignedToName: room.participants[uid].name, status: "pending" }; 
            pIndex++; 
            if (pIndex >= pKeys.length) pIndex = 0; 
        } 
        
        database.ref("family_khatmas/" + currentFkRoomId).update({ status: "active", portions: portions }).then(() => {
            showToast("تم التوزيع.. بسم الله نبدأ!");
            if (distBtn) { distBtn.innerHTML = `<i class="fa-solid fa-shuffle"></i> توزيع الأجزاء وبدء الختمة`; distBtn.disabled = false; }
        }).catch((e) => {
        ...
        showToast("مشكلة في الاتصال بالإنترنت");
        if (distBtn) { distBtn.innerHTML = `<i class="fa-solid fa-shuffle"></i> توزيع الأجزاء وبدء الختمة`; distBtn.disabled = false; }
    }); 
} 
...
window.markPortionDone = function(portionId) { 
    triggerHaptic(); 
    if (!database || !currentFkRoomId) return; 
    database.ref(`family_khatmas/${currentFkRoomId}/portions/${portionId}`).update({ status: "completed" }).then(() => {
...
window.deleteFkHistoryRecord = function(index) { 
    triggerHaptic(); 
...
window.addEventListener("DOMContentLoaded", () => { 
    ...
    try { initPrayerSettings(); } catch (e) {} 
