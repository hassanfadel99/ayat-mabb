 ...
 * النسخة المحدثة (4.0) - إصلاح الكاش + الختمة العائلية + الـ GPS + الإشعارات
 */ 

let APP_CURRENT_VERSION = 4.0; 
...
function openGPSSetting() {
    ...
    try {
        AndroidBridge.openLocationSettings();
...
function checkCurrentRoom() {
    ...
    fkRoomListener = database.ref('family_khatmas/' + currentFkRoomId).on('value', snapshot => {
        ...
        // 🌟 إرسال الإشعارات بدون أخطاء (Try-Catch) 🌟
        try {
            if (!isFkInitialLoad) {
                if (previousRoomStatus === "waiting" && room.status === "active") {
                    AndroidBridge.sendLocalNotification("بدأت الختمة!", "لقد تم توزيع السور والأجزاء لختمة: " + room.khatmaName);
                }
                if (room.creator === fkMyUid && room.participants) {
                    ...
                    if (previousParticipantsCount !== -1 && currentCount > previousParticipantsCount) {
                        ...
                        AndroidBridge.sendLocalNotification("انضمام جديد", "لقد انضم " + newParticipantName + " إلى ختمة " + room.khatmaName);
                    }
                }
            }
        } catch(e) {}
