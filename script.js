/** 
 * تطبيق آيات - الملف البرمجي الشامل (النسخة 3.8 المباشرة والنهائية)
 */ 
...
function checkForUpdates() { 
    ...
    database.ref("app_settings").once("value").then((snapshot) => { 
        ...
        if (data && parseFloat(data.latest_version) > APP_CURRENT_VERSION) { 
            ...
            if (updatePopup) {
                updatePopup.classList.remove("hidden-popup"); 
            }
        } 
...
// 🌟 متغير هام لمعرفة نوع الصوت المفتوح حالياً 🌟
...
function cleanSurahName(name) { 
    if (!name) return "";
    return name.replace(/[ًٌٍَُِّْـٰ]/g, "").replace(/سورة/g, "").trim(); 
} 
...
// ==========================================
// 🌟 3. دوال التواصل المباشر مع الأندرويد 🌟
// (تستقبل الأوامر من الإشعار متجاوزة حظر الخلفية)
// ==========================================
window.androidToggleAudio = function() {
    try {
        if (currentPlayingType === 'quran') {
            const qa = document.getElementById("quran-audio");
            if (qa) { if (qa.paused) qa.play(); else qa.pause(); }
        } else if (currentPlayingType === 'dua') {
            const la = document.getElementById("lib-audio");
            if (la) { if (la.paused) la.play(); else la.pause(); }
        }
    } catch(e) {
        console.log("Error toggling audio from Android", e);
    }
};

window.androidStopAudio = function() {
    try {
        if (currentPlayingType === 'quran') {
            const qa = document.getElementById("quran-audio");
            if (qa) { qa.pause(); qa.currentTime = 0; }
        } else if (currentPlayingType === 'dua') {
            const la = document.getElementById("lib-audio");
            if (la) { la.pause(); la.currentTime = 0; }
        }
        
        // التأكد من اختفاء المشغلات العائمة
        const fp = document.getElementById("floating-quran-player"); 
        if (fp) fp.classList.add("hidden");
        const fl = document.getElementById("floating-lib-player"); 
        if (fl) fl.classList.add("hidden");
        const fm = document.getElementById("full-audio-player-modal"); 
        if (fm) fm.classList.add("hidden");
    } catch(e) {
        console.log("Error stopping audio from Android", e);
    }
};

// ==========================================
// 4. تشغيل التطبيق والتنقل الأساسي
// ==========================================
...
window.showSection = function (id) { 
    triggerHaptic(); 
    const sb = document.getElementById("sidebar"); 
    if (sb && !sb.classList.contains("close")) {
        toggleSidebar(); 
    }
...
const hueSlider = document.getElementById("hue-slider"); 
if (hueSlider) { 
    hueSlider.addEventListener("input", function (e) { applyHueColor(e.target.value, false); }); 
} 
...
function loadPreferences() { 
    ...
    if (localStorage.getItem("ayat_darkmode") === "true") {
        setAppMode("night"); 
    } else {
        setAppMode("day"); 
    }
...
const iraqCitiesCoords = { 
    Basra: { lat: 30.5081, lng: 47.7835 }, Baghdad: { lat: 33.3152, lng: 44.3661 }, 
    Najaf: { lat: 31.9925, lng: 44.3258 }, Karbala: { lat: 32.616, lng: 44.0249 }, 
    Babil: { lat: 32.4682, lng: 44.4009 }, "Dhi Qar": { lat: 31.058, lng: 46.2573 }, 
    Maysan: { lat: 31.841, lng: 47.1432 }, "Al Qadisiyyah": { lat: 31.9868, lng: 44.9215 }, 
    "Al Muthanna": { lat: 31.3129, lng: 45.2818 }, Wasit: { lat: 32.5065, lng: 45.8231 }, 
    Diyala: { lat: 33.7431, lng: 44.6074 }, Kirkuk: { lat: 35.4674, lng: 44.3828 }, 
    Mosul: { lat: 36.34, lng: 43.13 }, Erbil: { lat: 36.1901, lng: 44.0092 }, 
    Sulaymaniyah: { lat: 35.55, lng: 45.4333 }, Saladin: { lat: 34.6071, lng: 43.6844 }, 
...
function calculateNextPrayer() { 
    ...
    const prayersToAlert = { 
        "الفجر": cleanPrayerTime(prayerTimesData.Fajr), 
        "الظهر": cleanPrayerTime(prayerTimesData.Dhuhr), 
        "المغرب": cleanPrayerTime(prayerTimesData.Maghrib) 
    }; 
    ...
    if (alarmPrayers.length > 0) { 
        ...
        if (nextAlarm.name !== lastScheduledPrayer) { 
            ...
            if (typeof window.AndroidBridge !== "undefined" && typeof window.AndroidBridge.schedulePrayerAlarm === "function") { 
...
function updateAdhanToggles() { 
    ...
    if (typeof window.AndroidBridge !== "undefined" && typeof window.AndroidBridge.cancelPrayerAlarm === "function") { 
...
// ==========================================
// 7. البث المباشر (الديناميكي)
// ==========================================
window.playDynamicLiveChannel = function(channelKey, defaultUrl, btnElement) { 
    triggerHaptic(); 
    document.querySelectorAll('.tv-btn').forEach(btn => btn.classList.remove('active')); 
    btnElement.classList.add('active'); 
    
    const iframe = document.getElementById('live-video-player'); 
    ...
    if (database) { 
        database.ref("live_tv/" + channelKey).once("value").then((snapshot) => { 
            const dynamicUrl = snapshot.val(); 
            if (dynamicUrl && dynamicUrl.trim() !== "") { 
                if (iframe) iframe.src = dynamicUrl; 
            } else { 
...
// ==========================================
// 8. مشغل القرآن الكريم (مع التزامن للأندرويد)
// ==========================================
...
window.startQuranAudio = function() { 
    ...
    quranAudioEl.play().then(() => {
        ...
        if (typeof window.AndroidBridge !== "undefined" && typeof window.AndroidBridge.startBackgroundAudio === "function") {
            window.AndroidBridge.startBackgroundAudio(`سورة ${currentSurahName}`, readerName);
        }
...
if (quranAudioEl) { 
    ...
    quranAudioEl.onplay = () => { 
        ...
        // تزامن الإشعار
        if (typeof window.AndroidBridge !== "undefined" && typeof window.AndroidBridge.updateAudioState === "function") {
...
window.stopQuranAudio = function() { 
    ...
    if (typeof window.AndroidBridge !== "undefined" && typeof window.AndroidBridge.stopBackgroundAudio === "function") {
        window.AndroidBridge.stopBackgroundAudio();
    }
} 

// ==========================================
// 9. مكتبة الأدعية والمشغل الكامل (مع التزامن للأندرويد)
// ==========================================
...
const audioLibraryData = {
    athkar: [ { title: "أذكار الصباح", reader: "مرتضى قريش", fileName: "sabah.mp3", cover: defaultCover }, { title: "أذكار المساء", reader: "حسين خليل", fileName: "massa.mp3", cover: defaultCover } ],
    duaa_days: [ { title: "دعاء يوم السبت", reader: "مرتضى قريش", fileName: "dye1.mp3", cover: defaultCover }, { title: "دعاء يوم الأحد", reader: "مرتضى قريش", fileName: "dye2.mp3", cover: defaultCover }, { title: "دعاء يوم الإثنين", reader: "مرتضى قريش", fileName: "dye3.mp3", cover: defaultCover }, { title: "دعاء يوم الثلاثاء", reader: "مرتضى قريش", fileName: "dye4.mp3", cover: defaultCover }, { title: "دعاء يوم الأربعاء", reader: "مرتضى قريش", fileName: "dye5.mp3", cover: defaultCover }, { title: "دعاء يوم الخميس", reader: "مرتضى قريش", fileName: "dye6.mp3", cover: defaultCover }, { title: "دعاء يوم الجمعة", reader: "مرتضى قريش", fileName: "dye7.mp3", cover: defaultCover } ],
    duaa_general: [ { title: "دعاء التوسل", reader: "مرتضى قريش", fileName: "duaa1.mp3", cover: defaultCover }, { title: "دعاء الندبة", reader: "مرتضى قريش", fileName: "duaa2.mp3", cover: defaultCover }, { title: "دعاء أبي حمزة الثمالي", reader: "مرتضى قريش", fileName: "duaa3.mp3", cover: defaultCover }, { title: "دعاء الإمام السجاد", reader: "مرتضى قريش", fileName: "duaa4.mp3", cover: defaultCover }, { title: "دعاء السمات", reader: "مرتضى قريش", fileName: "duaa5.mp3", cover: defaultCover }, { title: "دعاء العهد", reader: "مرتضى قريش", fileName: "duaa6.mp3", cover: defaultCover }, { title: "دعاء الفرج", reader: "مرتضى قريش", fileName: "duaa7.mp3", cover: defaultCover }, { title: "دعاء المجير", reader: "مرتضى قريش", fileName: "duaa8.mp3", cover: defaultCover }, { title: "دعاء زمن الغيبة", reader: "مرتضى قريش", fileName: "duaa9.mp3", cover: defaultCover }, { title: "دعاء كميل بن زياد", reader: "مرتضى قريش", fileName: "duaa10.mp3", cover: defaultCover }, { title: "دعاء مكارم الأخلاق", reader: "مرتضى قريش", fileName: "duaa11.mp3", cover: defaultCover }, { title: "دعاء يا عدتي", reader: "مرتضى قريش", fileName: "duaa12.mp3", cover: defaultCover } ],
    mnajat: [ { title: "اعتراف", reader: "مرتضى قريش", fileName: "mnajat1.mp3", cover: defaultCover }, { title: "مناجاة الخائفين", reader: "مرتضى قريش", fileName: "mnajat2.mp3", cover: defaultCover }, { title: "مناجاة الشاكين", reader: "مرتضى قريش", fileName: "mnajat3.mp3", cover: defaultCover }, { title: "مناجاة المريدين", reader: "مرتضى قريش", fileName: "mnajat4.mp3", cover: defaultCover }, { title: "مناجاة المفتقرين", reader: "مرتضى قريش", fileName: "mnajat5.mp3", cover: defaultCover }, { title: "مناجاة أمير المؤمنين", reader: "مرتضى قريش", fileName: "mnajat6.mp3", cover: defaultCover } ],
    ziyarats: [ { title: "الزيارة الجامعة الكبيرة", reader: "مرتضى قريش", fileName: "ziyarat1.mp3", cover: defaultCover }, { title: "زيارة آل ياسين", reader: "مرتضى قريش", fileName: "ziyarat2.mp3", cover: defaultCover }, { title: "زيارة الإمام الحسين (ع)", reader: "مرتضى قريش", fileName: "ziyarat3.mp3", cover: defaultCover }, { title: "زيارة الإمام المهدي (عج)", reader: "مرتضى قريش", fileName: "ziyarat4.mp3", cover: defaultCover }, { title: "زيارة الإمام علي (ع)", reader: "مرتضى قريش", fileName: "ziyarat5.mp3", cover: defaultCover }, { title: "زيارة الناحية المقدسة", reader: "مرتضى قريش", fileName: "ziyarat6.mp3", cover: defaultCover }, { title: "زيارة عاشوراء", reader: "مرتضى قريش", fileName: "ziyarat7.mp3", cover: defaultCover }, { title: "زيارة وارث", reader: "مرتضى قريش", fileName: "ziyarat8.mp3", cover: defaultCover } ]
};
...
window.playCustomAudioFile = function(fileName, title, reader, cover) { 
    ...
    libAudioEl.play().then(() => { 
        if (typeof window.AndroidBridge !== "undefined" && typeof window.AndroidBridge.startBackgroundAudio === "function") {
            window.AndroidBridge.startBackgroundAudio(title, reader);
        }
...
if (libAudioEl) { 
    ...
    libAudioEl.onpause = () => { 
        updateFullPlayerUI(); 
        cancelAnimationFrame(fpAnimationFrameId); 
        if (typeof window.AndroidBridge !== "undefined" && typeof window.AndroidBridge.updateAudioState === "function") {
            window.AndroidBridge.updateAudioState(false);
        }
    }; 
...
// 🌟 دالة تحميل الصوت الجديدة التي ترسل الرابط للأندرويد وتظهر الشريط 🌟
window.downloadAudioFile = function(event, url, title) { 
    ...
    if (!url.startsWith("http")) {
        fullUrl = window.location.origin + window.location.pathname.replace("index.html", "") + url;
    }

    if (typeof window.AndroidBridge !== "undefined" && typeof window.AndroidBridge.downloadAudio === "function") { 
        window.AndroidBridge.downloadAudio(fullUrl, title); 
        return; 
    } 

    downloadAbortController = new AbortController(); 
    const signal = downloadAbortController.signal; 
    fetch(fullUrl, { signal }).then(async response => { 
        ...
        const blobUrl = window.URL.createObjectURL(new Blob(chunks, { type: "audio/mp3" })); 
...
// ==========================================
// 10. القرآن الكريم، السور والصفحات
// ==========================================
async function loadSurahs() { 
    try { 
        const response = await fetch("https://api.alquran.cloud/v1/surah"); 
        const data = await response.json(); 
        allSurahs = data.data; renderSurahs(allSurahs); 
...
window.selectSurah = async function(number, name, savedPage = 0) { 
    ...
    try { 
        ...
        const data = await res.json(); 
        createPages(data.data.ayahs); currentPage = savedPage; renderPage(); saveReadingProgress(number, currentSurahName, savedPage); 
    } catch (error) { 
...
function renderPage() { 
    ...
    txt.innerHTML = ""; const p = pages[currentPage]; if (!p) return; 
    p.forEach((a) => { 
        const sp = document.createElement("span"); 
        sp.className = "ayah-box"; sp.style.fontSize = defaultFontSize + "px"; 
...
window.changeFontSize = function(amt) { 
    triggerHaptic(); defaultFontSize += amt; 
    if (defaultFontSize < 18) defaultFontSize = 18; if (defaultFontSize > 48) defaultFontSize = 48; 
    const t = document.getElementById("surah-text"); if (t) t.style.fontSize = defaultFontSize + "px"; 
...
window.resumeLastReading = function() { 
    ...
    if (d) { const p = JSON.parse(d); showSection("quran-section"); selectSurah(p.surahNum, p.surahName, p.pageIndex); } 
} 

window.updateWardGoal = function() { 
    triggerHaptic(); 
    const gInp = document.getElementById("setting-ward-goal"); const g = gInp ? parseInt(gInp.value) || 5 : 5; 
    localStorage.setItem("ward_pages_goal", g); updateWardDashboardProgress(); showToast("تم الحفظ"); 
} 
...
function updateWardDashboardProgress() { 
    ...
    const wStatus = document.getElementById("ward-status-text"); if (wStatus) wStatus.innerText = `تم قراءة ${rd.length} صفحات`; 
    const wPercent = document.getElementById("ward-percent-badge"); if (wPercent) wPercent.innerText = `${p}%`; 
    const wBar = document.getElementById("ward-bar-fill"); if (wBar) wBar.style.width = `${p}%`; 
...
function trackPageReadProgress() { 
    ...
    if (!rd.includes(uid)) { rd.push(uid); localStorage.setItem(dKey, JSON.stringify(rd)); updateWardDashboardProgress(); } 
} 

window.incrementMisbaha = function() { 
    misbahaCounter++; const num = document.getElementById("misbaha-count-number"); 
    if (num) num.innerText = misbahaCounter; if (navigator.vibrate) navigator.vibrate(45); 
} 
window.resetMisbaha = function() { 
    triggerHaptic(); misbahaCounter = 0; const num = document.getElementById("misbaha-count-number"); 
    if (num) num.innerText = misbahaCounter; 
} 
window.calculateKhatmaPlan = function() { 
    const input = document.getElementById("khatma-days-input"); const needed = document.getElementById("khatma-pages-needed"); 
    if (!input || !needed) return; const d = parseInt(input.value); 
    if (isNaN(d) || d <= 0) { needed.innerText = "0 صفحة"; return; } needed.innerText = `${Math.ceil(604 / d)} صفحة`; 
} 
window.activateKhatmaChallenge = function() { 
    triggerHaptic(); const inp = document.getElementById("khatma-days-input"); if (!inp) return; 
    const d = parseInt(inp.value); if (isNaN(d) || d <= 0) { showToast("الرجاء إدخال مدة صحيحة"); return; } 
...
window.switchFinancialTab = function(t) { 
    ...
    const tBtn = document.getElementById(`f-tab-${t}`); if (tBtn) tBtn.classList.add("active"); 
    const tCon = document.getElementById(`financial-${t}-content`); if (tCon) tCon.classList.remove("hidden"); 
    if (t === "records") renderFinancialHistory(); 
} 
window.calculateZakat = function() { 
    const inp = document.getElementById("zakat-amount-input"); if (!inp) return; const a = parseFloat(inp.value); 
    if (isNaN(a) || a <= 0) return; const z = a * 0.025; const zOut = document.getElementById("zakat-output"); if (zOut) zOut.innerText = z.toFixed(2); 
} 
window.calculateKhums = function() { 
    const inp = document.getElementById("khums-amount-input"); if (!inp) return; const a = parseFloat(inp.value); 
    if (isNaN(a) || a <= 0) return; const k = a * 0.2; const kOut = document.getElementById("khums-total-output"); if (kOut) kOut.innerText = k.toFixed(2); 
...
window.saveToFinancialHistory = function(type) { 
    ...
    if (type === "زكاة") { const zInp = document.getElementById("zakat-amount-input"); const zOut = document.getElementById("zakat-output"); amt = zInp ? parseFloat(zInp.value) : 0; right = zOut ? parseFloat(zOut.innerText) : 0; } 
    else { const kInp = document.getElementById("khums-amount-input"); const kOut = document.getElementById("khums-total-output"); amt = kInp ? parseFloat(kInp.value) : 0; right = kOut ? parseFloat(kOut.innerText) : 0; } 
...
window.closeVersePopup = function() { triggerHaptic(); const vPopup = document.getElementById("verse-action-popup"); if (vPopup) vPopup.classList.add("hidden-popup"); } 
window.copySelectedVerse = function() { triggerHaptic(); navigator.clipboard.writeText(`﴿ ${tempSelectedVerseText} ﴾ [ ${tempSelectedVerseInfo} ]`).then(() => { showToast("تم النسخ"); closeVersePopup(); }); } 
...
let fkMyUid = localStorage.getItem("fk_my_uid"); if (!fkMyUid) { fkMyUid = "user_" + Math.random().toString(36).substr(2, 9); localStorage.setItem("fk_my_uid", fkMyUid); } 
let currentFkRoomId = localStorage.getItem("fk_current_room") || null; let fkRoomListener = null; 
