/** 
 * تطبيق آيات - الملف البرمجي الشامل (النسخة السحابية المتزامنة ☁️)
 */ 
...
let currentUserUid = null; // 🌟 متغير تتبع هوية المستخدم في السحابة 🌟
...
let misbahaSyncTimeout = null; // لتأخير الرفع السحابي وتخفيف الضغط
...
window.incrementMisbaha = function() { 
    ...
    syncMisbahaToCloud(); // المزامنة مع فايربيس
} 
...
function syncMisbahaToCloud() {
    if (!currentUserUid || !database) return;
    clearTimeout(misbahaSyncTimeout);
    // تجميع الضغطات وإرسالها للسحابة كل ثانيتين لتجنب الضغط على السيرفر
    misbahaSyncTimeout = setTimeout(() => {
        database.ref("users/" + currentUserUid).update({ misbaha_total: totalMisbahaCounter }).catch(()=>{});
    }, 2000);
}
    ...
    if (deleteActionTarget === "single" && deleteTargetIndex !== null) { 
        ...
        if (currentFkRoomId && database) { 
            database.ref('family_khatmas/' + currentFkRoomId).once('value').then(snapshot => { 
                ...
                if (currentUserUid && database) database.ref("users/" + currentUserUid).child("fk_current_room").remove();
                showToast("تم الخروج من الختمة بنجاح"); switchFkTab('create'); 
            }); 
        } 
        ...
        syncFkHistoryToCloud(history);
        loadFkHistory(); showToast("تم مسح السجل بنجاح"); 
    } 
    closeDeletePopup(); 
} 

function syncFinancialToCloud(records) {
    if (currentUserUid && database) {
        database.ref("users/" + currentUserUid).update({ financial_records: records }).catch(()=>{});
    }
}
...
// ==========================================
// 🌟 13. الختمة العائلية والمزامنة السحابية 🌟
// ==========================================
...
window.createFamilyRoom = function() { 
    ...
    database.ref("family_khatmas/" + roomId).set(roomData).then(() => { 
        ...
        if (currentUserUid) database.ref("users/" + currentUserUid).update({ fk_current_room: roomId }).catch(()=>{});
...
window.joinFamilyRoom = function() { 
    ...
    database.ref("family_khatmas/" + code).once("value").then((snapshot) => { 
        ...
        database.ref("family_khatmas/" + code + "/participants/" + fkMyUid).set({ name: name }).then(() => { 
            ...
            if (currentUserUid) database.ref("users/" + currentUserUid).update({ fk_current_room: code }).catch(()=>{});
...
function saveToFkHistory(room) { 
    ...
    if(khatmaCountEl) khatmaCountEl.innerText = history.length;
} 

function syncFkHistoryToCloud(history) {
    if (currentUserUid && database) {
        database.ref("users/" + currentUserUid).update({ fk_history: history }).catch(()=>{});
    }
}
...
// 🌟 فحص حالة المستخدم والمزامنة السحابية المباشرة 🌟
function checkAuthState() {
    ...
    firebase.auth().onAuthStateChanged((user) => {
        ...
        if (user) {
            ...
            fkMyUid = user.uid; // توحيد هوية الختمة مع السحابة
            ...
            // 🌟 استدعاء البيانات من السحابة وتحديث الجهاز بها 🌟
            if (database) {
                database.ref("users/" + user.uid).once("value").then((snapshot) => {
                    const cloudData = snapshot.val() || {};

                    // 1. مزامنة التسبيحات
                    if (cloudData.misbaha_total !== undefined) {
                        totalMisbahaCounter = cloudData.misbaha_total;
                        localStorage.setItem("ayat_total_misbaha", totalMisbahaCounter);
                    }
                    document.getElementById("profile-zekr-count").innerText = totalMisbahaCounter;

                    // 2. مزامنة الحقوق الشرعية
                    if (cloudData.financial_records) {
                        localStorage.setItem("financial_history_records", JSON.stringify(cloudData.financial_records));
                        renderFinancialHistory();
                    }

                    // 3. مزامنة سجل الختمات
                    if (cloudData.fk_history) {
                        localStorage.setItem("fk_history", JSON.stringify(cloudData.fk_history));
                        loadFkHistory();
                        document.getElementById("profile-khatma-count").innerText = cloudData.fk_history.length;
                    } else {
                        document.getElementById("profile-khatma-count").innerText = "0";
                    }

                    // 4. استعادة غرفة الختمة إذا كان مسجلاً بها
                    if (cloudData.fk_current_room) {
                        currentFkRoomId = cloudData.fk_current_room;
                        localStorage.setItem("fk_current_room", currentFkRoomId);
                        checkCurrentRoom();
                    }
                }).catch(()=>{});
            }
            ...
            avatarInput.addEventListener("change", function(e) {
                ...
                reader.onload = function(event) {
                    ...
                    img.onload = function() {
                        ...
                        if (database) {
                            database.ref("users/" + user.uid).update({ photoURL: dataUrl })
                            .then(() => showToast("✅ تم تحديث الصورة الشخصية بنجاح!"))
                            .catch(() => showToast("✅ تم الحفظ محلياً بنجاح"));
                        }
                    };
