...
// 🌟 قراءة رقم الإصدار ديناميكياً بأقوى طريقة
...
function checkForUpdates() {
  // 🌟 الشرط الذهبي: إذا لم يكن هناك "جسر أندرويد"، فهذا يعني أننا في الموقع العادي، توقف ولا تظهر التحديث!
  ...
  database.ref('app_settings').once('value').then((snapshot) => {
    ...
    // 🌟 فايربيس يقارن رقمه بالرقم الفعلي القادم من الموبايل
...
function executeAppUpdate() {
  triggerHaptic();
  if (dynamicUpdateUrl) {
    // نتحقق من وجود تطبيق الأندرويد ونرسل الرابط له ليفتحه في كروم إجبارياً
    if (typeof AndroidBridge !== "undefined" && typeof AndroidBridge.openUpdateLink === "function") {
...
const i18n = { 
  ar: { 
    ...
    about_desc: "تطبيق آيات هو رفيقك الرقمي لرحلة إيمانية متكاملة، يجمع بين بساطة التصميم وعمق المحتوى.", copy: "نسخ النص", close: "إغلاق", share: "مشاركة التطبيق", 
    stories: "ستوريات قرآنية", stories_desc: "تلاوات قصيرة" 
  },
  en: {
    ...
    about_desc: "Ayat App is your digital companion for an integrated faith journey.", copy: "Copy Text", close: "Close", share: "Share App",
    stories: "Quran Stories", stories_desc: "Short Recitations"
  },
  ku: {
    ...
    about_desc: "پڕۆگرامی ئايات هاوڕێی دیجیتاڵیی تۆیە بۆ گەشتێکی ئیمانی.", copy: "لەبەرگرتنەوە", close: "داخستن", share: "بڵاوکردنەوە",
    stories: "ستۆری قورئانی", stories_desc: "خوێندنەوەی کورت"
  },
  fa: {
    ...
    about_desc: "برنامه آیات همراه دیجیتال شما در یک سفر ایمانی است.", copy: "کپی متن", close: "بستن", share: "اشتراک گذاری",
    stories: "استوری‌های قرآنی", stories_desc: "تلاوت‌های کوتاه"
  }
}; 
...
const asmaUlHusna = [ 
  ...
  { name: "الْحَكِيمُ", meaning: "الذي يضع الأشياء في مواضعها.", quran: "سورة الأنعام - الآية 18", ayah: "﴿وَهُوَ الْحَكِيمُ الْخَبِير﴾" }, 
  ...
  { name: "الْحَمِيدُ", meaning: "المستحق للحمد والثنائي.", quran: "سورة البقرة - الآية 267", ayah: "﴿وَاعْلَمُوا أَنَّ اللَّهَ غَنِيٌّ حَمِيدٌ﴾" }, 
...
let allStoriesData = []; let currentPlayingVideo = null; let downloadAbortController = null; 
...
function setupNameOfDay() { 
  ...
  const el = document.getElementById("nod-name"); if (el && nod) el.innerText = nod.name; 
} 
...
function startAudio() { 
  triggerHaptic(); const readerSelect = document.getElementById("reader-select"); 
...
function stopAudio() { 
  triggerHaptic(); 
  if (quranAudioEl) { quranAudioEl.pause(); quranAudioEl.currentTime = 0; cancelAnimationFrame(animationFrameId); } 
...
window.startApp = function () { 
  ...
  if (splash) { 
    ...
    setTimeout(() => { 
      ...
      const appC = document.getElementById("app-content"); if (appC) appC.classList.remove("hidden"); 
...
window.showSection = function (id) { 
  triggerHaptic(); const sb = document.getElementById("sidebar"); 
  if (sb && !sb.classList.contains("close")) toggleSidebar(); 
  if (id !== "stories-section") { 
    document.querySelectorAll(".story-video").forEach((vid) => { 
      vid.pause(); const overlay = document.getElementById(vid.id.replace("story-vid", "story-overlay")); 
...
function backToList() { 
  triggerHaptic(); const panel = document.getElementById("surah-selector-panel"); const zone = document.getElementById("reading-zone"); 
  if (panel) panel.classList.remove("hidden"); if (zone) zone.classList.add("hidden"); 
} 

async function loadSurahs() { 
  try { 
    const response = await fetch("https://api.alquran.cloud/v1/surah"); 
    const data = await response.json(); allSurahs = data.data; renderSurahs(allSurahs); 
  } catch (e) { 
    console.error(e);
...
function renderSurahs(list) { 
  ...
  list.forEach((s) => { 
    const btn = document.createElement("button"); btn.className = "surah-grid-item haptic-btn"; 
...
async function selectSurah(number, name, savedPage = 0) { 
  ...
  const panel = document.getElementById("surah-selector-panel"); const zone = document.getElementById("reading-zone"); 
  if (panel) panel.classList.add("hidden"); if (zone) zone.classList.remove("hidden"); 
  const st = document.getElementById("surah-title"); if (st) st.innerHTML = `سورة ${currentSurahName}`; 
...
function renderPage() { 
  const txt = document.getElementById("surah-text"); if (!txt || !pages.length) return; txt.innerHTML = ""; 
  const p = pages[currentPage]; if (!p) return; 
  p.forEach((a) => { 
    const sp = document.createElement("span"); sp.className = "ayah-box"; sp.style.fontSize = defaultFontSize + "px"; 
  ...
  const ind = document.getElementById("page-indicator"); if (ind) ind.innerHTML = `صفحة ${currentPage + 1} من ${pages.length}`; 
  const prevBtn = document.getElementById("prev-page-btn"); if (prevBtn) prevBtn.disabled = currentPage === 0; 
  const nextBtn = document.getElementById("next-page-btn"); if (nextBtn) nextBtn.disabled = currentPage === pages.length - 1; 
...
function updateWardGoal() { 
  triggerHaptic(); const gInp = document.getElementById("setting-ward-goal"); const g = gInp ? parseInt(gInp.value) || 5 : 5; 
...
function updateWardDashboardProgress() { 
  ...
  if (svd && resBox) { 
    const pr = JSON.parse(svd); const resTxt = document.getElementById("resume-text"); 
...
function trackPageReadProgress() { 
  ...
  if (!rd.includes(uid)) { 
    rd.push(uid); localStorage.setItem(dKey, JSON.stringify(rd)); updateWardDashboardProgress(); 
  } 
} 

function switchAzkarCategory(cat) { 
  triggerHaptic(); currentAzkarCategory = cat; 
  document.querySelectorAll(".azkar-category-tabs-scroll .tab-btn").forEach((b) => b.classList.remove("active")); 
  const t = document.getElementById(`tab-${cat}`); if (t) t.classList.add("active"); 
...
function renderAzkarList(list) { 
  ...
  list.forEach((z, i) => { 
    const d = document.createElement("div"); d.className = "zekr-card"; 
...
function filterAzkar() { 
  ...
  else renderAzkarList(fullAzkarDatabase[currentAzkarCategory].filter((z) => z.text.toLowerCase().includes(q))); 
} 

function countZekr(cat, i) { 
  triggerHaptic(); const b = document.getElementById(`counter-${cat}-${i}`); if (!b) return; 
  let n = parseInt(b.innerText.replace(/[^\d]/g, "")); 
  if (n > 1) { n--; b.innerText = `المتبقي: ${n}`; } 
  else { b.innerText = "تمت القراءة ✓"; b.className = "zekr-counter-btn done"; b.disabled = true; } 
} 
...
function activateKhatmaChallenge() { 
  triggerHaptic(); const inp = document.getElementById("khatma-days-input"); if (!inp) return; const d = parseInt(inp.value); 
...
function checkKhatmaStatusOnDashboard() { 
  ...
  const dText = document.getElementById("dashboard-khatma-text"); const dAlert = document.getElementById("dashboard-khatma-alert"); 
  if (k && dText && dAlert) { 
    const pk = JSON.parse(k); dText.innerText = `المطلوب: ${pk.pagesPerDay} صفحة يومياً`; dAlert.classList.remove("hidden"); 
  } 
} 
...
function calculateZakat() { 
  ...
  if (isNaN(a) || a <= 0) return; const z = a * 0.025; const zOut = document.getElementById("zakat-output"); 
...
function calculateKhums() { 
  ...
  if (isNaN(a) || a <= 0) return; const k = a * 0.2; const kOut = document.getElementById("khums-total-output"); 
  if (kOut) kOut.innerText = k.toFixed(2); const kImam = document.getElementById("khums-imam-output"); 
  if (kImam) kImam.innerText = (k / 2).toFixed(2); const kSad = document.getElementById("khums-sadah-output"); 
...
function saveToFinancialHistory(type) { 
  ...
  if (type === "زكاة") { 
    const zInp = document.getElementById("zakat-amount-input"); const zOut = document.getElementById("zakat-output"); 
    amt = zInp ? parseFloat(zInp.value) : 0; right = zOut ? parseFloat(zOut.innerText) : 0; 
  } else { 
    const kInp = document.getElementById("khums-amount-input"); const kOut = document.getElementById("khums-total-output"); 
  ...
  localStorage.setItem("financial_history_records", JSON.stringify(h)); showToast("تم الحفظ"); 
} 
...
function renderFinancialHistory() { 
  ...
  const h = JSON.parse(localStorage.getItem("financial_history_records")) || []; 
  if (h.length === 0) { 
    c.innerHTML = `<div style="text-align: center; opacity: 0.7; padding: 20px; font-family: 'Cairo'; color: var(--text);">لا توجد سجلات محفوظة حالياً</div>`; return; 
  } 
  h.forEach((i, index) => { 
...
function deleteFinancialRecord(index) { 
  ...
  const msg = document.getElementById("delete-popup-msg"); if (msg) msg.innerText = "هل أنت متأكد من حذف هذا السجل بشكل نهائي؟"; 
...
function clearFinancialHistory() { 
  ...
  const msg = document.getElementById("delete-popup-msg"); if (msg) msg.innerText = "هل أنت متأكد من مسح جميع السجلات؟ لا يمكن التراجع عن هذه الخطوة."; 
  const popup = document.getElementById("delete-confirm-popup"); if (popup) popup.classList.remove("hidden-popup"); 
} 

function closeDeletePopup() { 
  triggerHaptic(); const p = document.getElementById("delete-confirm-popup"); if (p) p.classList.add("hidden-popup"); 
...
function executeDelete() { 
  ...
  if (deleteActionTarget === "single" && deleteTargetIndex !== null) { 
    const h = JSON.parse(localStorage.getItem("financial_history_records")) || []; h.splice(deleteTargetIndex, 1); 
    localStorage.setItem("financial_history_records", JSON.stringify(h)); renderFinancialHistory(); showToast("تم حذف السجل"); 
...
window.toggleFavorite = function (event, url, index) { 
  ...
  try { 
    ...
    if (favs.includes(url)) { 
      favs = favs.filter((u) => u !== url); btn.classList.remove("active"); btn.innerHTML = `<i class="fa-regular fa-heart"></i>`; showToast("تم الإزالة من المفضلة"); 
    } else { 
      favs.push(url); btn.classList.add("active"); btn.innerHTML = `<i class="fa-solid fa-heart text-red" style="color: #e63946;"></i>`; showToast("تمت الإضافة للمفضلة ❤️"); 
    } 
...
function setupVideoControls(index) { 
  ...
  video.addEventListener("timeupdate", () => { 
    if (!seekbar.isDragging) { const progress = (video.currentTime / video.duration) * 100; seekbar.value = progress || 0; if (currTimeText) currTimeText.innerText = formatTime(video.currentTime); } 
  }); 
...
window.toggleStoryVideo = function (index) { 
  ...
  if (video.paused) { 
    document.querySelectorAll(".story-video").forEach((vid) => { 
      if (vid !== video) { 
        vid.pause(); const ov = document.getElementById(vid.id.replace("vid", "overlay")); if (ov) ov.classList.remove("playing"); 
        const mb = document.getElementById(vid.id.replace("story-vid-", "vid-play-btn-")); if (mb) mb.innerHTML = '<i class="fa-solid fa-play"></i>'; 
      } 
    }); 
    video.play().catch((e) => console.log("Play blocked")); 
    if (overlay) overlay.classList.add("playing"); if (miniPlayBtn) miniPlayBtn.innerHTML = '<i class="fa-solid fa-pause"></i>'; currentPlayingVideo = video; 
  } else { 
  ...
  if ("IntersectionObserver" in window) { 
    const observer = new IntersectionObserver((entries) => { 
      entries.forEach((entry) => { 
        ...
        } else { vid.pause(); vid.currentTime = 0; if (overlay) overlay.classList.remove("playing"); if (miniPlayBtn) miniPlayBtn.innerHTML = '<i class="fa-solid fa-play"></i>'; } 
      }); 
    }, { threshold: 0.5 }); 
...
window.shareApp = function () { 
  triggerHaptic(); const shareText = "تطبيق آيات: رحلة إيمانية متكاملة في القرآن الكريم، الأذكار، والستوريات.\nhttps://hassanfadel99.github.io/ayat-mabb/"; 
  if (typeof AndroidBridge !== "undefined") { AndroidBridge.shareApp(shareText); } else if (navigator.share) { navigator.share({ title: "تطبيق آيات", text: shareText }).catch((e) => {}); } else { navigator.clipboard.writeText(shareText).then(() => showToast("تم نسخ الرابط للحافظة")); } 
}; 
...
window.directDownloadVideo = async function (event, url) { 
  ...
  if (typeof AndroidBridge !== "undefined") { AndroidBridge.downloadVideo(url); return; } 
...
window.cancelDownload = function () { 
  triggerHaptic(); if (typeof AndroidBridge !== "undefined") { AndroidBridge.cancelDownloadVideo(); } else if (downloadAbortController) { downloadAbortController.abort(); } 
