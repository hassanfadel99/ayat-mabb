/** 
 * تطبيق آيات - الملف البرمجي الشامل (النسخة الأصلية المحدثة بالتقويم)
 */ 
...
// 🌟 إعدادات التاريخ الهجري 🌟
let hijriAdjustment = parseInt(localStorage.getItem('ayat_hijri_adj')) || 0;
...
// ==========================================
// 6. نظام مواقيت الصلاة والأذان (المحدث بدقة الثواني)
// ==========================================
...
// 🌟 مؤقت الأذان الدقيق المتزامن مع الثواني 🌟
function startAccuratePrayerClock() { 
    if (prayerTimeout) clearTimeout(prayerTimeout);
    function tick() {
        calculateNextPrayer();
        let now = new Date();
        let msToNextSecond = 1000 - now.getMilliseconds();
        prayerTimeout = setTimeout(tick, msToNextSecond);
    }
    tick(); 
} 
...
function calculateNextPrayer() { 
    ...
    // تشغيل الأذان في نفس اللحظة (الثانية 0)
    for (let [prayerName, prayerTime] of Object.entries(prayersToAlert)) { 
        if (adhanToggles[prayerName] && currentHHMM === prayerTime && currentSS === 0 && (lastAdhanPlayed !== prayerTime || lastAdhanPlayedDay !== currentDay)) { 
...
window.updateHijriAdjustment = function(val) {
    triggerHaptic();
    hijriAdjustment = parseInt(val) || 0;
...
function updateCalendarView() {
    // 1. التاريخ الهجري (يتأثر بالإضافة والنقصان)
    let adjustedDate = new Date();
    adjustedDate.setDate(adjustedDate.getDate() + hijriAdjustment); 
    
    // 2. 🌟 التاريخ الميلادي (الحقيقي الثابت دائماً) 🌟
    ...
    if(heroGreg) {
        // نستخدم الوقت الحقيقي (realDate) حتى لا يتغير الميلادي
...
// ==========================================
// 13. الختمة العائلية (مع تصليح التجميد)
// ==========================================
