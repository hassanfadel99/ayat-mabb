...
// 🌟 متغيرات نظام التحديث الديناميكي
...
// 🌟 دالة فحص التحديثات من فايربيس
function checkForUpdates() {
  if (!database) return;
  database.ref('app_settings').once('value').then((snapshot) => {
    const data = snapshot.val();
    // نستخدم parseFloat لضمان قراءة الرقم بشكل صحيح حتى لو تم حفظه كنص
...
function executeAppUpdate() {
  triggerHaptic();
  if (dynamicUpdateUrl) {
    window.open(dynamicUpdateUrl, '_blank');
  }
  closeUpdatePopup();
}
